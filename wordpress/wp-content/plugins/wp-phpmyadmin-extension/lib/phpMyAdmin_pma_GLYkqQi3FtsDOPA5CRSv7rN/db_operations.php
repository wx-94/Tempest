...
if (strlen($GLOBALS['db']) > 0
    ...
    if (! isset($_REQUEST['newname']) || strlen($_REQUEST['newname']) === 0) {
        $message = Message::error(__PMA_TRANSL('The database name is empty!'));
    } else {
        ...
        if ($_REQUEST['newname'] === $_REQUEST['db']) {
            $message = Message::error(
                __PMA_TRANSL('Cannot copy database to the same name. Change the name and try again.')
            );
        } else {
            $_error = false;
            ...
            if (! $_error && $move) {
                ...
                $message = Message::success(
                    __PMA_TRANSL('Database %1$s has been renamed to %2$s.')
                );
                ...
                $message = Message::success(
                    __PMA_TRANSL('Database %1$s has been copied to %2$s.')
                );
                $message->addParam($GLOBALS['db']);
                $message->addParam($_REQUEST['newname']);
            } else {
