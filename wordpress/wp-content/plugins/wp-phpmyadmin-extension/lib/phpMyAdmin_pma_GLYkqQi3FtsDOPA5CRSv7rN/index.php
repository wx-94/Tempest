...
if (isset($_SESSION['partial_logout'])) {
    Message::success(
        __PMA_TRANSL('You were logged out from one server, to logout completely from phpMyAdmin, you need to logout from all servers.')
    )->display();
...
if ($server > 0 || count($cfg['Servers']) > 1
    ...
    if ($cfg['DBG']['demo']) {
        ...
        echo '<h2>' , __PMA_TRANSL('phpMyAdmin Demo Server') , '</h2>';
    ...
    echo '<div class="group">';
    echo '<h2>' , __PMA_TRANSL('General settings') , '</h2>';
    echo '<ul>';
    ...
    if ($server > 0) {
        ...
        if ($cfg['Server']['auth_type'] != 'config') {
            if ($cfg['ShowChgPassword']) {
                ...
                Core::printListItem(
                    Util::getImage('s_passwd') . "&nbsp;" . __PMA_TRANSL(
                        'Change password'
                    ),
                    'li_change_password',
        ...
        echo '        <form class="disableAjax" method="post" action="index.php">' , "\n"
           ...
           . '                ' . Util::getImage('s_asci')
            . "&nbsp;" . __PMA_TRANSL('Server connection collation') . "\n"
...
echo '<div class="group">';
echo '<h2>' , __PMA_TRANSL('Appearance settings') , '</h2>';
echo '  <ul>';
...
if ($server > 0) {
    echo '<ul>';
    Core::printListItem(
        Util::getImage('b_tblops') . "&nbsp;" . __PMA_TRANSL(
            'More settings'
        ),
        'li_user_preferences',
...
if ($server > 0 && $GLOBALS['cfg']['ShowServerInfo']) {
    ...
    echo '<div class="group">';
    echo '<h2>' , __PMA_TRANSL('Database server') , '</h2>';
    echo '<ul>' , "\n";
    Core::printListItem(
        __PMA_TRANSL('Server:') . ' ' . $server_info,
        'li_server_info'
    );
    Core::printListItem(
        __PMA_TRANSL('Server type:') . ' ' . Util::getServerType(),
        'li_server_type'
    );
    Core::printListItem(
        __PMA_TRANSL('Server connection:') . ' ' . Util::getServerSSL(),
        'li_server_type'
    );
    Core::printListItem(
        __PMA_TRANSL('Server version:')
        . ' '
        . $GLOBALS['dbi']->getVersionString() . ' - ' . $GLOBALS['dbi']->getVersionComment(),
        'li_server_version'
    );
    Core::printListItem(
        __PMA_TRANSL('Protocol version:') . ' ' . $GLOBALS['dbi']->getProtoInfo(),
        'li_mysql_proto'
    );
    Core::printListItem(
        __PMA_TRANSL('User:') . ' ' . htmlspecialchars($mysql_cur_user_and_host),
        'li_user_info'
    );

    echo '    <li id="li_select_mysql_charset">';
    echo '        ' , __PMA_TRANSL('Server charset:') , ' '
...
if ($GLOBALS['cfg']['ShowServerInfo'] || $GLOBALS['cfg']['ShowPhpInfo']) {
    echo '<div class="group">';
    echo '<h2>' , __PMA_TRANSL('Web server') , '</h2>';
    echo '<ul>';
    if ($GLOBALS['cfg']['ShowServerInfo']) {
        ...
        if ($server > 0) {
            ...
            Core::printListItem(
                __PMA_TRANSL('Database client version:') . ' ' . $client_version_str,
                'li_mysql_client_version'
            );

            $php_ext_string = __PMA_TRANSL('PHP extension:') . ' ';
            ...
            $php_version_string = __PMA_TRANSL('PHP version:') . ' ' . phpversion();
    ...
    if ($cfg['ShowPhpInfo']) {
        Core::printListItem(
            __PMA_TRANSL('Show PHP information'),
            'li_phpinfo',
...
Core::printListItem(
    __PMA_TRANSL('Version information:') . ' <span class="version">' . PMA_VERSION . '</span>',
    'li_pma_version',
    null,
    null,
    null,
...
Core::printListItem(
    __PMA_TRANSL('Official Homepage'),
    'li_pma_homepage',
...
if (is_null($GLOBALS['PMA_Config']->getTempDir('twig'))) {
    trigger_error(
        sprintf(
            __PMA_TRANSL('The $cfg[\'TempDir\'] (%s) is not accessible. phpMyAdmin is not able to cache templates and will be slow because of this.'),
