<?php
					...
					//$wpinc = "wp-includes" ;
					//renaming function require_once( ABSPATH . WPINC . "/l10n.php" );  // <--- impossible. better to rename pma"s __PMA_TRANSL() func.
...
if (! defined('PMA_MINIMUM_COMMON')) {
    ...
    if (! empty($cfg['Server'])) {
        ...
        if (! @class_exists($auth_class)) {
            Core::fatalError(
                __PMA_TRANSL('Invalid authentication method set in configuration:')
        ...
        if ($GLOBALS['dbi']->getVersion() < $cfg['MysqlMinVersion']['internal']) {
            Core::fatalError(
                __PMA_TRANSL('You should upgrade to %s %s or later.'),
    ...
    if ($response->isAjax() && $_SERVER['REQUEST_METHOD'] == 'POST' && $token_mismatch) {
        ...
        $response->addJSON(
            'message',
            Message::error(__PMA_TRANSL('Error: Token mismatch'))
        );
        exit;
    }
}
