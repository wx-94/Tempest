...
class InsertEdit
{
    ...
    private function showEmptyResultMessageOrSetUniqueCondition(
        ...
        if (! $rows[$key_id]) {
            ...
            Response::getInstance()->addHtml(
                Util::getMessage(
                    __PMA_TRANSL('MySQL returned an empty result set (i.e. zero rows).'),
                    $local_query
                )
            );
            /**
    ...
    private function showTypeOrFunctionLabel($which)
    {
        switch ($which) {
            case 'function':
                return __PMA_TRANSL('Function');
            case 'type':
                return __PMA_TRANSL('Type');
        }
    ...
    private function getFunctionColumn(
        ...
        if (($GLOBALS['cfg']['ProtectBinary'] === 'blob'
            ...
            $html_output .= '<td class="center">' . __PMA_TRANSL('Binary') . '</td>' . "\n";
        } elseif ($readOnly
    ...
    private function getBinaryAndBlobColumn(
        ...
        if (($GLOBALS['cfg']['ProtectBinary'] === 'blob' && $column['is_blob'])
            ...
            $html_output .= __PMA_TRANSL('Binary - do not edit');
            if (isset($data)) {
    ...
    private function getSelectOptionForUpload($vkey, array $column)
    {
        ...
        if ($files === false) {
            return '<span style="color:red">' . __PMA_TRANSL('Error') . '</span><br />' . "\n"
                .  __PMA_TRANSL('The directory you set for upload work cannot be reached.') . "\n";
        } elseif (!empty($files)) {
            return "<br />\n"
                . '<i>' . __PMA_TRANSL('Or') . '</i>' . ' '
                . __PMA_TRANSL('web server upload directory:') . '<br />' . "\n"
    ...
    {
        $edit_str = Util::getIcon('b_edit', __PMA_TRANSL('Edit/Insert'));
    ...
    private function getSubmitTypeDropDown(
        ...
        if (isset($where_clause)) {
            $html_output .= '<option value="save">' . __PMA_TRANSL('Save') . '</option>';
        }
        $html_output .= '<option value="insert">'
            . __PMA_TRANSL('Insert as new row')
            . '</option>'
            . '<option value="insertignore">'
            . __PMA_TRANSL('Insert as new row and ignore errors')
            . '</option>'
            . '<option value="showinsert">'
            . __PMA_TRANSL('Show insert query')
            . '</option>'
            . '</select>';
        return $html_output;
    }
        ...
        if (isset($where_clause)) {
            ...
            if ($found_unique_key && $is_numeric) {
                $html_output .= '<option value="edit_next" '
                    ...
                    . __PMA_TRANSL('Edit next row') . '</option>';
            }
        }
        $html_output .= '</select>';
        return $html_output;
    ...
    private function getSubmitAndResetButtonForActionsPanel($tabindex, $tabindex_for_value)
    {
        ...
        . '<input type="submit" class="control_at_footer" value="' . __PMA_TRANSL('Go') . '"'
        . ' tabindex="' . ($tabindex + $tabindex_for_value + 6) . '" id="buttonYes" />'
        . '<input type="button" class="preview_sql" value="' . __PMA_TRANSL('Preview SQL') . '"'
        . ' tabindex="' . ($tabindex + $tabindex_for_value + 7) . '" />'
        . '<input type="reset" class="control_at_footer" value="' . __PMA_TRANSL('Reset') . '"'
    ...
    private function getHeadAndFootOfInsertRowTable(array $url_params)
    {
        ...
        $html_output .= '<th>' . __PMA_TRANSL('Null') . '</th>'
            . '<th>' . __PMA_TRANSL('Value') . '</th>'
            . '</tr>'
            . '</thead>'
            . ' <tfoot>'
            . '<tr>'
    ...
    public function executeSqlQuery(array $url_params, array $query)
    {
        ...
        foreach ($query as $single_query) {
            if ($_REQUEST['submit_type'] == 'showinsert') {
                $last_messages[] = Message::notice(__PMA_TRANSL('Showing SQL query'));
                continue;
            }
            ...
            if (! $result) {
                ...
                if ($insert_id != 0) {
                    ...
                    $last_message = Message::notice(__PMA_TRANSL('Inserted row id: %1$d'));
