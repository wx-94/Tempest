...
class Menu
{
        ...
        if (strlen($this->_db) > 0) {
            ...
            if (strlen($this->_table) > 0
                ...
                $retval .= sprintf(
                    ...
                    $tbl_is_view ? __PMA_TRANSL('View') : __PMA_TRANSL('Table')
                );
                ...
                if (! empty($show_comment)
                    ...
                    $retval .= sprintf(
                        __PMA_TRANSL('“%s”'), htmlspecialchars($show_comment)
                    );
                    $retval .= '</span>';
                } // end if
            } else {
    ...
    private function _getTableTabs()
    {
        ...
        $tabs['sql']['text'] = __PMA_TRANSL('SQL');

        $tabs['search']['icon'] = 'b_search';
        $tabs['search']['text'] = __PMA_TRANSL('Search');
        ...
        if (! $db_is_system_schema && (! $tbl_is_view || $updatable_view)) {
            ...
            $tabs['insert']['text'] = __PMA_TRANSL('Insert');
        }

        $tabs['export']['icon'] = 'b_tblexport';
        $tabs['export']['link'] = 'tbl_export.php';
        $tabs['export']['args']['single_table'] = 'true';
        $tabs['export']['text'] = __PMA_TRANSL('Export');
        ...
        if (! $tbl_is_view && ! $db_is_system_schema) {
            ...
            $tabs['import']['text'] = __PMA_TRANSL('Import');
        }
        if (($is_superuser || $isCreateOrGrantUser)
            ...
            $tabs['privileges']['text'] = __PMA_TRANSL('Privileges');
        ...
        if (! $tbl_is_view && ! $db_is_system_schema) {
            ...
            $tabs['operation']['text'] = __PMA_TRANSL('Operations');
        }
        /**
        ...
        if (Tracker::isActive() && ! $db_is_system_schema) {
            ...
            $tabs['tracking']['text'] = __PMA_TRANSL('Tracking');
        ...
        if (! $db_is_system_schema
            ...
            $tabs['triggers']['text'] = __PMA_TRANSL('Triggers');
    ...
    private function _getDbTabs()
    {
        ...
        if ($num_tables == 0) {
            $tabs['search']['warning'] = __PMA_TRANSL('Database seems to be empty!');
        }

        $tabs['multi_table_query']['text'] = __PMA_TRANSL('Query');
        $tabs['multi_table_query']['icon'] = 's_db';
        $tabs['multi_table_query']['link'] = 'db_multi_table_query.php';
        if ($num_tables == 0) {
            $tabs['qbe']['warning'] = __PMA_TRANSL('Database seems to be empty!');
        }
        ...
        if ($num_tables == 0) {
            $tabs['export']['warning'] = __PMA_TRANSL('Database seems to be empty!');
        }
        ...
        if (! $db_is_system_schema) {
            $tabs['designer']['text'] = __PMA_TRANSL('Designer');
        ...
        if (! $db_is_system_schema
            ...
            $tabs['central_columns']['text'] = __PMA_TRANSL('Central columns');
    ...
    private function _getServerTabs()
    {
        ...
        $tabs['databases']['text'] = __PMA_TRANSL('Databases');
        ...
        $tabs['status']['text'] = __PMA_TRANSL('Status');
        ...
        if ($is_superuser || $isCreateOrGrantUser) {
            ...
            $tabs['rights']['text'] = __PMA_TRANSL('User accounts');
        ...
        $tabs['settings']['text']   = __PMA_TRANSL('Settings');
        ...
        if (! empty($binary_logs)) {
            ...
            $tabs['binlog']['text'] = __PMA_TRANSL('Binary log');
        }

        if ($is_superuser) {
            $tabs['replication']['icon'] = 's_replication';
            $tabs['replication']['link'] = 'server_replication.php';
            $tabs['replication']['text'] = __PMA_TRANSL('Replication');
        }

        $tabs['vars']['icon'] = 's_vars';
        $tabs['vars']['link'] = 'server_variables.php';
        $tabs['vars']['text'] = __PMA_TRANSL('Variables');

        $tabs['charset']['icon'] = 's_asci';
        $tabs['charset']['link'] = 'server_collations.php';
        $tabs['charset']['text'] = __PMA_TRANSL('Charsets');

        $tabs['engine']['icon'] = 'b_engine';
        $tabs['engine']['link'] = 'server_engines.php';
        $tabs['engine']['text'] = __PMA_TRANSL('Engines');

        $tabs['plugins']['icon'] = 'b_plugin';
        $tabs['plugins']['link'] = 'server_plugins.php';
        $tabs['plugins']['text'] = __PMA_TRANSL('Plugins');
