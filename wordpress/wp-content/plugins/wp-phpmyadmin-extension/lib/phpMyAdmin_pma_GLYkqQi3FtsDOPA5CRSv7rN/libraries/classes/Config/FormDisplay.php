...
class FormDisplay
{
    ...
    public function __construct(ConfigFile $cf)
    {
        $this->_jsLangStrings = array(
            'error_nan_p' => __PMA_TRANSL('Not a positive number!'),
            'error_nan_nneg' => __PMA_TRANSL('Not a non-negative number!'),
            'error_incorrect_port' => __PMA_TRANSL('Not a valid port number!'),
            'error_invalid_value' => __PMA_TRANSL('Incorrect value!'),
            'error_value_lte' => __PMA_TRANSL('Value must be equal or lower than %s!'));
        $this->_configFile = $cf;
            ...
            foreach ($form->fields as $field => $system_path) {
                ...
                if (!isset($_POST[$key])) {
                    ...
                    if ($type == 'boolean') {
                        ...
                        $this->_errors[$form->name][] = sprintf(
                            __PMA_TRANSL('Missing data for %s'),
                ...
                case 'select':
                    ...
                    if (! $successfully_validated) {
                        $this->_errors[$work_path][] = __PMA_TRANSL('Incorrect value!');
                        $result = false;
                        continue;
                    }
                    break;
    ...
    private function _setComments($system_path, array &$opts)
    {
        ...
        if ($system_path == 'RecodingEngine') {
            ...
            if (!function_exists('iconv')) {
                $opts['values']['iconv'] .= ' (' . __PMA_TRANSL('unavailable') . ')';
                $comment = sprintf(
                    __PMA_TRANSL('"%s" requires %s extension'), 'iconv', 'iconv'
                );
            }
            if (!function_exists('recode_string')) {
                $opts['values']['recode'] .= ' (' . __PMA_TRANSL('unavailable') . ')';
                $comment .= ($comment ? ", " : '') . sprintf(
                    __PMA_TRANSL('"%s" requires %s extension'),
                    'recode', 'recode'
                );
            }
        ...
        if (! $GLOBALS['PMA_Config']->get('is_setup')) {
            if (($system_path == 'MaxDbList' || $system_path == 'MaxTableList'
                ...
                $opts['comment'] = sprintf(
                    __PMA_TRANSL('maximum %s'), $GLOBALS['cfg'][$system_path]
                );
            }
        }
    }
