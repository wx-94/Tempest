...
abstract class AuthenticationPlugin
{
    ...
    public function getErrorMessage($failure)
    {
        if ($failure == 'empty-denied') {
            ...
            return __PMA_TRANSL('Access denied!');
        } elseif ($failure == 'no-activity') {
            return sprintf(
                __PMA_TRANSL('No activity within %s seconds; please log in again.'),
        ...
        if (!empty($dbi_error)) {
            ...
            . __PMA_TRANSL('Cannot log in to the MySQL server');
        }

        return __PMA_TRANSL('Cannot log in to the MySQL server');
    }
    ...
    public function checkTwoFactor()
    {
        ...
        Message::rawNotice(
            __PMA_TRANSL('You have enabled two factor authentication, please confirm your login.')
        )->display();
