...
class Navigation
{
    ...
    public function getDisplay()
    {
        ...
        if (! $navRender) {
            $retval .= Message::error(
                __PMA_TRANSL('An error has occurred while loading the navigation display')
            )->getDisplay();
        } else {
            $retval .= $navRender;
        }
    ...
    public function getItemUnhideDialog($dbName, $itemType = null, $tableName = null)
    {
        ...
        $typeMap = array(
            'group' => __PMA_TRANSL('Groups:'),
            'event' => __PMA_TRANSL('Events:'),
            'function' => __PMA_TRANSL('Functions:'),
            'procedure' => __PMA_TRANSL('Procedures:'),
            'table' => __PMA_TRANSL('Tables:'),
            'view' => __PMA_TRANSL('Views:'),
        );
        if (empty($tableName)) {
            $first = true;
                ...
                if ((empty($itemType) || $itemType == $t)
                    ...
                    foreach ($hidden[$t] as $hiddenItem) {
                        ...
                        $html .= '<td style="width:80px"><a href="navigation.php'
                            ...
                            . Util::getIcon('show.png', __PMA_TRANSL('Show'))
                            .  '</a></td>';
                    }
                    $html .= '</tbody></table>';
                    $first = false;
