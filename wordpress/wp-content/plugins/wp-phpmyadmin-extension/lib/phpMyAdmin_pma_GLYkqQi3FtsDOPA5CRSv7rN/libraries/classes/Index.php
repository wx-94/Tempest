...
class Index
{
    ...
    public static function getHtmlForDisplayIndexes()
    {
        ...
        $html_output .= sprintf(
            __PMA_TRANSL('Create an index on &nbsp;%s&nbsp;columns'),
        ...
        $html_output .= '<input type="hidden" name="create_index" value="1" />'
            ...
            . ' type="submit" value="' . __PMA_TRANSL('Go') . '" />';
    ...
    static public function getHtmlForIndexes($table, $schema, $print_mode = false)
    {
        ...
        $no_indexes .= Message::notice(__PMA_TRANSL('No index defined!'))->getDisplay();
        $no_indexes .= '</div>';

        if (! $print_mode) {
            $r  = '<fieldset class="index_info">';
            $r .= '<legend id="index_header">' . __PMA_TRANSL('Indexes');
        ...
        $r .= '<tr>';
        if (! $print_mode) {
            $r .= '<th colspan="2" class="print_ignore">' . __PMA_TRANSL('Action') . '</th>';
        }
        $r .= '<th>' . __PMA_TRANSL('Keyname') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Type') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Unique') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Packed') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Column') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Cardinality') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Collation') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Null') . '</th>';
        $r .= '<th>' . __PMA_TRANSL('Comment') . '</th>';
        $r .= '</tr>';
        $r .= '</thead>';
            ...
            if (! $print_mode) {
                ...
                if ($index->getName() == 'PRIMARY') {
                    ...
                    $this_params['message_to_show']
                        = __PMA_TRANSL('The primary key has been dropped.');
                    ...
                    $this_params['message_to_show'] = sprintf(
                        __PMA_TRANSL('Index %s has been dropped.'), htmlspecialchars($index->getName())
                    );
                    $js_msg = Sanitize::jsFormat($this_params['sql_query']);
                }
                ...
                $r .= Util::linkOrButton(
                    ...
                    Util::getIcon('b_drop', __PMA_TRANSL('Drop')),
