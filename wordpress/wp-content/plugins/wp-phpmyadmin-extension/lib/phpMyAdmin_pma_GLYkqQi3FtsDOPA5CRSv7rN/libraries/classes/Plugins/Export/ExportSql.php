...
class ExportSql extends ExportPlugin
{
    ...
    protected function setProperties()
    {
        ...
        if (!$hide_sql) {
            ...
            $leaf = new TextPropertyItem(
                'header_comment',
                __PMA_TRANSL('Additional custom header comment (\n splits lines):')
            );
            ...
            if (!empty($GLOBALS['cfgRelation']['relation'])) {
                $leaf = new BoolPropertyItem(
                    'relation',
                    __PMA_TRANSL('Display foreign key relationships')
                );
                $subgroup->addProperty($leaf);
            }
            if (!empty($GLOBALS['cfgRelation']['mimework'])) {
                $leaf = new BoolPropertyItem(
                    'mime',
                    __PMA_TRANSL('Display MIME types')
                );
                $subgroup->addProperty($leaf);
            }
            ...
            $leaf = new BoolPropertyItem(
                "use_transaction",
                __PMA_TRANSL('Enclose export in a transaction')
            );
            $leaf->setDoc(
                array(
                    'programs',
            ...
            $leaf = new BoolPropertyItem(
                "disable_fk",
                __PMA_TRANSL('Disable foreign key checks')
            );
            $leaf->setDoc(
                array(
            ...
            $leaf = new BoolPropertyItem(
                "views_as_tables",
                __PMA_TRANSL('Export views as tables')
            );
            $generalOptions->addProperty($leaf);
            ...
            $subgroup = new OptionsPropertySubgroup(
                "dump_table", __PMA_TRANSL("Dump table")
            );
            ...
            if (!$hide_structure) {
                $structureOptions = new OptionsPropertyMainGroup(
                    "structure", __PMA_TRANSL('Object creation options')
                );
                ...
                if ($plugin_param['export_type'] == 'server') {
                    $leaf = new BoolPropertyItem(
                        "drop_database",
                        sprintf(__PMA_TRANSL('Add %s statement'), '<code>DROP DATABASE IF EXISTS</code>')
                    );
                    $subgroup->addProperty($leaf);
                }
                ...
                if ($plugin_param['export_type'] == 'database') {
                    ...
                    $leaf = new BoolPropertyItem(
                        'create_database',
                        sprintf(__PMA_TRANSL('Add %s statement'), $create_clause)
                    );
                    $subgroup->addProperty($leaf);
                }
                ...
                $leaf = new BoolPropertyItem(
                    'drop_table',
                    sprintf(__PMA_TRANSL('Add %s statement'), $drop_clause)
                );
                $subgroup->addProperty($leaf);
                ...
                $leaf = new BoolPropertyItem(
                    'create_table',
                    sprintf(__PMA_TRANSL('Add %s statement'), '<code>CREATE TABLE</code>')
                );
                $subgroup_create_table->setSubgroupHeader($leaf);

                $leaf = new BoolPropertyItem(
                    'if_not_exists',
                    '<code>IF NOT EXISTS</code> ' . __PMA_TRANSL(
                ...
                $leaf = new BoolPropertyItem(
                    'auto_increment',
                    sprintf(__PMA_TRANSL('%s value'), '<code>AUTO_INCREMENT</code>')
                );
                $subgroup_create_table->addProperty($leaf);
                ...
                $leaf = new BoolPropertyItem(
                    'create_view',
                    sprintf(__PMA_TRANSL('Add %s statement'), '<code>CREATE VIEW</code>')
                );
                $subgroup->addProperty($leaf);
                ...
                $leaf = new BoolPropertyItem(
                    'create_trigger',
                    sprintf(__PMA_TRANSL('Add %s statement'), '<code>CREATE TRIGGER</code>')
                );
                $subgroup->addProperty($leaf);
            ...
            $dataOptions = new OptionsPropertyMainGroup(
                "data", __PMA_TRANSL('Data creation options')
            );
            $dataOptions->setForce('structure');
            $leaf = new BoolPropertyItem(
                "truncate",
                __PMA_TRANSL('Truncate table before insert')
            );
            ...
            $subgroup = new OptionsPropertySubgroup();
            $leaf = new MessageOnlyPropertyItem(
                __PMA_TRANSL('Instead of <code>INSERT</code> statements, use:')
            );
            $subgroup->setSubgroupHeader($leaf);

            $leaf = new BoolPropertyItem(
                "delayed",
                __PMA_TRANSL('<code>INSERT DELAYED</code> statements')
            );
            $leaf->setDoc(
                array(
                    'manual_MySQL_Database_Administration',
            ...
            $leaf = new BoolPropertyItem(
                "ignore",
                __PMA_TRANSL('<code>INSERT IGNORE</code> statements')
            );
            $leaf->setDoc(
                array(
                    'manual_MySQL_Database_Administration',
            ...
            $leaf = new SelectPropertyItem(
                "type",
                __PMA_TRANSL('Function to use when dumping data:')
            );
            $leaf->setValues(
                array(
                    'INSERT'  => 'INSERT',
            ...
            $leaf = new MessageOnlyPropertyItem(
                null,
                __PMA_TRANSL('Syntax to use when inserting data:')
            );
            $subgroup->setSubgroupHeader($leaf);
            ...
            $leaf = new NumberPropertyItem(
                "max_query_size",
                __PMA_TRANSL('Maximal length of created query')
            );
            $dataOptions->addProperty($leaf);
    ...
    protected function _exportRoutineSQL(
        ...
        if ($used_alias) {
            $text .= $this->_exportComment(
                __PMA_TRANSL('It appears your database uses routines;')
            )
            . $this->_exportComment(
                __PMA_TRANSL('alias export may not work reliably in all cases.')
            )
            . $this->_exportComment();
        }
        $text .= $proc_query;
    ...
    public function exportHeader()
    {
        ...
        $head = $this->_exportComment('phpMyAdmin SQL Dump')
            ...
            . $this->_exportComment();
        $host_string = __PMA_TRANSL('Host:') . ' ' . $cfg['Server']['host'];
        if (!empty($cfg['Server']['port'])) {
            $host_string .= ':' . $cfg['Server']['port'];
        }
        $head .= $this->_exportComment($host_string);
        $head .= $this->_exportComment(
            __PMA_TRANSL('Generation Time:') . ' '
            . Util::localisedDate()
        )
        . $this->_exportComment(
            __PMA_TRANSL('Server version:') . ' ' . $GLOBALS['dbi']->getVersionString()
        )
        . $this->_exportComment(__PMA_TRANSL('PHP Version:') . ' ' . phpversion())
        . $this->_possibleCRLF();
    ...
    public function exportEvents($db)
    {
        ...
        if ($event_names) {
            ...
            $text .= $this->_exportComment()
                . $this->_exportComment(__PMA_TRANSL('Events'))
                . $this->_exportComment();
    ...
    public function exportMetadata(
        ...
        $comment = $this->_possibleCRLF()
            . $this->_possibleCRLF()
            . $this->_exportComment()
            . $this->_exportComment(__PMA_TRANSL('Metadata'))
            . $this->_exportComment();
    ...
    private function _exportMetadata(
        ...
        if (isset($table)) {
            $comment .= $this->_exportComment(
                sprintf(
                    __PMA_TRANSL('Metadata for table %s'),
                    $table
                )
            );
        } else {
            $comment .= $this->_exportComment(
                sprintf(
                    __PMA_TRANSL('Metadata for database %s'),
                    $db
                )
            );
        }
    ...
    public function getTableDef(
        ...
        if ($tmp_error) {
            $message = sprintf(__PMA_TRANSL('Error reading structure for table %s:'), "$db.$table");
            $message .= ' ' . $tmp_error;
        ...
        if ($result != false && ($row = $GLOBALS['dbi']->fetchRow($result))) {
            ...
            if ($flag && $view) {
                $warning = $this->_exportComment()
                    . $this->_exportComment(
                        __PMA_TRANSL('It appears your database uses views;')
                    )
                    . $this->_exportComment(
                        __PMA_TRANSL('alias export may not work reliably in all cases.')
                    )
            ...
            if (!empty($statement->fields) && (empty($engine) || strtoupper($engine) != 'ARCHIVE')) {
                ...
                if (!empty($constraints)) {
                    ...
                    $sql_constraints = $this->generateComment(
                        $crlf,
                        $sql_constraints,
                        __PMA_TRANSL('Constraints for dumped tables'),
                        __PMA_TRANSL('Constraints for table'),
                        $table_alias,
                        $compat
                ...
                if ((!empty($indexes)) || (!empty($indexes_fulltext))) {
                    $sql_indexes = $this->generateComment(
                        $crlf,
                        $sql_indexes,
                        __PMA_TRANSL('Indexes for dumped tables'),
                        __PMA_TRANSL('Indexes for table'),
                        $table_alias,
                        $compat
                    ) . $sql_indexes_query;
                }
                ...
                if ((! empty($auto_increment)) && ($update_indexes_increments)) {
                    ...
                    $sql_auto_increments = $this->generateComment(
                        $crlf,
                        $sql_auto_increments,
                        __PMA_TRANSL('AUTO_INCREMENT for dumped tables'),
                        __PMA_TRANSL('AUTO_INCREMENT for table'),
                        $table_alias,
                        $compat
    ...
    private function _getTableComments(
        ...
        if (isset($mime_map) && count($mime_map) > 0) {
            $schema_create .= $this->_possibleCRLF()
                . $this->_exportComment()
                . $this->_exportComment(
                    __PMA_TRANSL('MIME TYPES FOR TABLE') . ' '
        ...
        if ($have_rel) {
            $schema_create .= $this->_possibleCRLF()
                . $this->_exportComment()
                . $this->_exportComment(
                    __PMA_TRANSL('RELATIONSHIPS FOR TABLE') . ' '
                    . Util::backquote($table_alias, $sql_backquotes)
                    . ':'
                );
    ...
    public function exportStructure(
        ...
        case 'create_table':
            $dump .= $this->_exportComment(
                __PMA_TRANSL('Table structure for table') . ' ' . $formatted_table_name
            );
            $dump .= $this->_exportComment();
        ...
        case 'triggers':
            ...
            if ($triggers) {
                $dump .= $this->_possibleCRLF()
                    . $this->_exportComment()
                    . $this->_exportComment(
                        __PMA_TRANSL('Triggers') . ' ' . $formatted_table_name
                    )
                    . $this->_exportComment();
                $used_alias = false;
                $trigger_query = '';
                ...
                if ($used_alias) {
                    $dump .= $this->_exportComment(
                        __PMA_TRANSL('It appears your table uses triggers;')
                    )
                    . $this->_exportComment(
                        __PMA_TRANSL('alias export may not work reliably in all cases.')
                    )
        ...
        case 'create_view':
            if (empty($GLOBALS['sql_views_as_tables'])) {
                $dump .= $this->_exportComment(
                    __PMA_TRANSL('Structure for view')
                    . ' '
                ...
                $dump .= $this->_exportComment(
                    sprintf(
                        __PMA_TRANSL('Structure for view %s exported as a table'),
                        $formatted_table_name
                    )
                )
                . $this->_exportComment();
        ...
        case 'stand_in':
            $dump .= $this->_exportComment(
                __PMA_TRANSL('Stand-in structure for view') . ' ' . $formatted_table_name
            )
                . $this->_exportComment(
                __PMA_TRANSL('(See below for the actual view)')
            )
                . $this->_exportComment();
    ...
    public function exportData(
        ...
        if ($GLOBALS['dbi']->getTable($db, $table)->isView()
            ...
            $head = $this->_possibleCRLF()
                ...
                . $this->_exportComment(__PMA_TRANSL('Data:') . ' ' . __PMA_TRANSL('None'))
                . $this->_exportComment()
                . $this->_possibleCRLF();
        ...
        $tmp_error = $GLOBALS['dbi']->getError();
        if ($tmp_error) {
            $message = sprintf(__PMA_TRANSL('Error reading data for table %s:'), "$db.$table");
            $message .= ' ' . $tmp_error;
            if (! defined('TESTSUITE')) {
                trigger_error($message, E_USER_ERROR);
            }
        ...
        while ($row = $GLOBALS['dbi']->fetchRow($result)) {
            if ($current_row == 0) {
                $head = $this->_possibleCRLF()
                    . $this->_exportComment()
                    . $this->_exportComment(
                        __PMA_TRANSL('Dumping data for table') . ' '
                        . $formatted_table_name
                    )
                    . $this->_exportComment()
                    . $crlf;
