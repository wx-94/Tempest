...
class AuthenticationCookie extends AuthenticationPlugin
{
    ...
    public function showLoginForm()
    {
        ...
        if ($GLOBALS['cfg']['DBG']['demo']) {
            echo '<fieldset>';
            echo '<legend>' , __PMA_TRANSL('phpMyAdmin Demo Server') , '</legend>';
            printf(
                __PMA_TRANSL(
        ...
        if (! empty($conn_error)) {
            ...
            Message::rawError(
                __PMA_TRANSL('Your session has expired. Please log in again.')
            )->display();
        }
    ...
    <form method="post" id="login_form" action="index.php" name="login_form"' , $autocomplete ,
        ...
        if ($GLOBALS['cfg']['AllowArbitraryServer']) {
            ...
            echo '<div class="item">
                <label for="input_username">' , __PMA_TRANSL('Username:') , '</label>
                <input type="text" name="pma_username" id="input_username" '
                , 'value="' , htmlspecialchars($default_user) , '" size="24"'
                , ' class="textfield"/>
            </div>
            <div class="item">
                <label for="input_password">' , __PMA_TRANSL('Password:') , '</label>
                <input type="password" name="pma_password" id="input_password"'
                , ' value="" size="24" class="textfield" />
            </div>';
        if (count($GLOBALS['cfg']['Servers']) > 1) {
            echo '<div class="item">
                <label for="select_server">' . __PMA_TRANSL('Server Choice:') . '</label>
        ...
        if (empty($GLOBALS['cfg']['CaptchaLoginPrivateKey'])
            ...
            echo '<input value="' , __PMA_TRANSL('Go') , '" type="submit" id="input_go" />';
        }
        else {
            ...
            , htmlspecialchars($GLOBALS['cfg']['CaptchaLoginPublicKey']),'"'
                .' data-callback="recaptchaCallback" value="' , __PMA_TRANSL('Go') , '" type="submit" id="input_go" />';
        }
        $_form_params = array();
                ...
                if (! empty($_POST["g-recaptcha-response"])) {
                    ...
                    if ($resp == null || ! $resp->isSuccess()) {
                        ...
                        if (in_array('invalid-json', $codes)) {
                            $conn_error = __PMA_TRANSL('Failed to connect to the reCAPTCHA service!');
                        } else {
                            $conn_error = __PMA_TRANSL('Entered captcha is wrong, try again!');
                        }
                        return false;
                    }
                } else {
                    $conn_error = __PMA_TRANSL('Missing reCAPTCHA verification, maybe it has been blocked by adblock?');
                    return false;
                }
            }
