...
class SavedSearches
{
    ...
    public function save()
    {
        if (null == $this->getSearchName()) {
            $message = Message::error(
                __PMA_TRANSL('Please provide a name for this bookmarked search.')
            );
        ...
        if (null == $this->getUsername()
            ...
            $message = Message::error(
                __PMA_TRANSL('Missing information to save the bookmarked search.')
            );
            $response = Response::getInstance();
            $response->setRequestStatus($message->isSuccess());
            $response->addJSON('message', $message);
        ...
        if (null === $this->getId()) {
            ...
            if (!empty($existingSearches)) {
                $message = Message::error(
                    __PMA_TRANSL('An entry with this name already exists.')
                );
                $response = Response::getInstance();
                $response->setRequestStatus($message->isSuccess());
                $response->addJSON('fieldWithError', 'searchName');
    ...
    public function delete()
    {
        if (null == $this->getId()) {
            $message = Message::error(
                __PMA_TRANSL('Missing information to delete the search.')
            );
            $response = Response::getInstance();
            $response->setRequestStatus($message->isSuccess());
    ...
    public function load()
    {
        if (null == $this->getId()) {
            $message = Message::error(
                __PMA_TRANSL('Missing information to load the search.')
            );
            $response = Response::getInstance();
            $response->setRequestStatus($message->isSuccess());
            $response->addJSON('fieldWithError', 'searchId');
        ...
        if (false === ($oneResult = $GLOBALS['dbi']->fetchArray($resList))) {
            $message = Message::error(__PMA_TRANSL('Error while loading the search.'));
            $response = Response::getInstance();
            $response->setRequestStatus($message->isSuccess());
            $response->addJSON('fieldWithError', 'searchId');
            $response->addJSON('message', $message);
