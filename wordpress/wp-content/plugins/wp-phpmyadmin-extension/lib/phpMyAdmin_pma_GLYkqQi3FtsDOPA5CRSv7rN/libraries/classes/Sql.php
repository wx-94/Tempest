...
class Sql
{
    ...
    private function getHtmlForRelationalColumnDropdown($db, $table, $column, $curr_value)
    {
        ...
        if ($foreignData['disp_row'] == null) {
            ...
            $dropdown = '<span class="curr_value">'
                ...
                . __PMA_TRANSL('Browse foreign values')
                . '</a>';
        } else {
    ...
    private function getHtmlForProfilingChart($url_query, $db, $profiling_results)
    {
        if (! empty($profiling_results)) {
            ...
            $profiling_table = '';
            $profiling_table .= '<fieldset><legend>' . __PMA_TRANSL('Profiling')
                . '</legend>' . "\n";
            $profiling_table .= '<div class="floatleft">';
            $profiling_table .= '<h3>' . __PMA_TRANSL('Detailed profile') . '</h3>';
            $profiling_table .= '<table id="profiletable"><thead>' . "\n";
            $profiling_table .= ' <tr>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('Order')
                . '<div class="sorticon"></div></th>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('State')
                . Util::showMySQLDocu('general-thread-states')
                . '<div class="sorticon"></div></th>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('Time')
                . '<div class="sorticon"></div></th>' . "\n";
            ...
            $profiling_table .= '<div class="floatleft">';
            $profiling_table .= '<h3>' . __PMA_TRANSL('Summary by state') . '</h3>';
            $profiling_table .= '<table id="profilesummarytable"><thead>' . "\n";
            $profiling_table .= ' <tr>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('State')
                . Util::showMySQLDocu('general-thread-states')
                . '<div class="sorticon"></div></th>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('Total Time')
                . '<div class="sorticon"></div></th>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('% Time')
                . '<div class="sorticon"></div></th>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('Calls')
                . '<div class="sorticon"></div></th>' . "\n";
            $profiling_table .= '  <th>' . __PMA_TRANSL('ø Time')
                . '<div class="sorticon"></div></th>' . "\n";
            $profiling_table .= ' </tr></thead><tbody>' . "\n";
...
EOT;
    ...
    public function getHtmlForBookmark(array $displayParts, array $cfgBookmark, $sql_query, $db,
        ...
        if ($displayParts['bkm_form'] == '1'
            ...
            $html .= '<legend>';
            $html .= Util::getIcon(
                'b_bookmark', __PMA_TRANSL('Bookmark this SQL query'), true
            );
            $html .= '</legend>';
            $html .= '<div class="formelement">';
            $html .= '<label>' . __PMA_TRANSL('Label:');
            ...
            $html .=  __PMA_TRANSL('Let every user access this bookmark') . '</label>';
            $html .= '</div>';
            ...
            $html .= '<input type="submit"'
                . ' value="' . __PMA_TRANSL('Bookmark this SQL query') . '" />';
            $html .= '</fieldset>';
            $html .= '</form>';
    ...
    public function addBookmark($goto)
    {
        ...
        if ($response->isAjax()) {
            if ($result) {
                $msg = Message::success(__PMA_TRANSL('Bookmark %s has been created.'));
                $msg->addParam($_POST['bkm_fields']['bkm_label']);
                $response->addJSON('message', $msg);
            } else {
                $msg = Message::error(__PMA_TRANSL('Bookmark not created!'));
                $response->setRequestStatus(false);
                $response->addJSON('message', $msg);
            }
            exit;
    ...
    public function getDefaultSqlQueryForBrowse($db, $table)
    {
        ...
        if (! empty($bookmark) && ! empty($bookmark->getQuery())) {
            $GLOBALS['using_bookmark_message'] = Message::notice(
                __PMA_TRANSL('Using bookmark "%s" as default browse query.')
            );
    ...
    private function getMessageForNoRowsReturned($message_to_show,
        ...
        if ($analyzed_sql_results['querytype'] == 'DELETE"') {
            ...
            if ($insert_id != 0) {
                ...
                // message
                $_inserted = Message::notice(__PMA_TRANSL('Inserted row id: %1$d'));
            ...
            $message = Message::success(__PMA_TRANSL('Showing as PHP code'));
        } elseif (isset($GLOBALS['show_as_php'])) {
            /* User disable showing as PHP, query is only displayed */
            $message = Message::notice(__PMA_TRANSL('Showing SQL query'));
        } else {
        ...
        if (isset($GLOBALS['querytime'])) {
            $_querytime = Message::notice(
                '(' . __PMA_TRANSL('Query took %01.4f seconds.') . ')'
            );
        ...
        if (isset($_REQUEST['rollback_query'])) {
            $message->addText(__PMA_TRANSL('[ROLLBACK occurred.]'));
        }
    ...
    private function getBookmarkCreatedMessage()
    {
        if (isset($_GET['label'])) {
            $bookmark_created_msg = Message::success(
                __PMA_TRANSL('Bookmark %s has been created.')
            );
            ...
            foreach ($selectedTables as $tbl_name) {
                ...
                if (! empty($check)) {
                    $indexes_problems_html .= sprintf(
                        __PMA_TRANSL('Problems with indexes of table `%s`'), $tbl_name
                    );
