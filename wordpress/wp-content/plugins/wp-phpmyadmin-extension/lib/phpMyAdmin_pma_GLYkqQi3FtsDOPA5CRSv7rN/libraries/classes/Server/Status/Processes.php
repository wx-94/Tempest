...
class Processes
{
    ...
    public static function getHtmlForProcessListAutoRefresh()
    {
        ...
        $retval .= '<label>' . __PMA_TRANSL('Refresh rate') . ': ';
        ...
        $retval .= Util::getImage('play') . __PMA_TRANSL('Start auto refresh');
        $retval .= '</a>';
        $retval .= '</div>';
        return $retval;
    }
    ...
    public static function getHtmlForServerProcesslist()
    {
        ...
        $sortable_columns = array(
            array(
                'column_name' => __PMA_TRANSL('ID'),
                'order_by_field' => 'Id'
            ),
            array(
                'column_name' => __PMA_TRANSL('User'),
                'order_by_field' => 'User'
            ),
            array(
                'column_name' => __PMA_TRANSL('Host'),
                'order_by_field' => 'Host'
            ),
            array(
                'column_name' => __PMA_TRANSL('Database'),
                'order_by_field' => 'db'
            ),
            array(
                'column_name' => __PMA_TRANSL('Command'),
                'order_by_field' => 'Command'
            ),
            array(
                'column_name' => __PMA_TRANSL('Time'),
                'order_by_field' => 'Time'
            ),
            array(
                'column_name' => __PMA_TRANSL('Status'),
                'order_by_field' => 'State'
            ),
            array(
                'column_name' => __PMA_TRANSL('Progress'),
                'order_by_field' => 'Progress'
            ),
            array(
                'column_name' => __PMA_TRANSL('SQL query'),
                'order_by_field' => 'Info'
            )
        );
        ...
        $retval .= '<thead>';
        $retval .= '<tr>';
        $retval .= '<th>' . __PMA_TRANSL('Processes') . '</th>';
        foreach ($sortable_columns as $column) {
            ...
            if ($is_sorted) {
                ...
                $retval .= '<img class="icon ic_s_desc soimg" alt="'
                    . __PMA_TRANSL('Descending') . '" title="" src="themes/dot.gif" '
                    . 'style="display: ' . $desc_display_style . '" />';
                $retval .= '<img class="icon ic_s_asc soimg hide" alt="'
                    . __PMA_TRANSL('Ascending') . '" title="" src="themes/dot.gif" '
            ...
            if (0 === --$sortableColCount) {
                ...
                if ($show_full_sql) {
                    $retval .= Util::getImage('s_partialtext', __PMA_TRANSL('Truncate Shown Queries'));
                } else {
                    $retval .= Util::getImage('s_fulltext', __PMA_TRANSL('Show Full Queries'));
                }
                $retval .= '</a>';
            }
            $retval .= '</th>';
    ...
    public static function getHtmlForProcessListFilter()
    {
        ...
        $retval .= __PMA_TRANSL('Show only active');
        $retval .= '</label>';
        $retval .= '</div>';
        $retval .= '</form>';
        $retval .= '</fieldset>';
