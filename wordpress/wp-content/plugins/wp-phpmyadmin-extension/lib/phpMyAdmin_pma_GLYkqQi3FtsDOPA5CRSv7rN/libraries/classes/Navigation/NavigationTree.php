...
class NavigationTree
{
        ...
        if ($node === false) {
            ...
            if (!$GLOBALS['cfg']['ShowDatabasesNavigationAsTree']) {
                ...
                if (empty($listContent)) {
                    ...
                    $retval .= __PMA_TRANSL('No tables found in database.');
                    $retval .= "</div>";
                }
            }
            $retval .= "</div>";
    ...
    private function _fastFilterHtml($node)
    {
        ...
        if ($node === $this->_tree
            ...
            $retval .= " placeholder='"
                . __PMA_TRANSL("Type to filter these, Enter to search all");
            $retval .= "' />";
            $retval .= '<span title="' . __PMA_TRANSL('Clear fast filter') . '">X</span>';
            $retval .= "</form>";
            $retval .= "</li>";
    ...
    private function _controls()
    {
        ...
        $retval .= Util::getNavigationLink(
            '#',
            $showText,
            __PMA_TRANSL('Collapse all'),
            $showIcon,
            's_collapseall',
            'pma_navigation_collapse'
        );
        $syncImage = 's_unlink';
        $title = __PMA_TRANSL('Link with main panel');
        if ($GLOBALS['cfg']['NavigationLinkWithMainPanel']) {
            $syncImage = 's_link';
            $title = __PMA_TRANSL('Unlink from main panel');
        }
        $retval .= Util::getNavigationLink(
            '#',
            $showText,
