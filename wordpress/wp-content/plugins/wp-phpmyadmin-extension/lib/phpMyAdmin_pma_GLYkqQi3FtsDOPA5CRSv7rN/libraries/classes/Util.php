...
class Util
{
    ...
    public static function getFormattedMaximumUploadSize($max_upload_size)
    {
        ...
        return '(' . sprintf(__PMA_TRANSL('Max: %s%s'), $max_size, $max_unit) . ')';
    }
    ...
    public static function showDocLink($link, $target = 'documentation', $bbcode = false)
    {
        ...
        return '<a href="' . $link . '" target="' . $target . '">'
            . self::getImage('b_help', __PMA_TRANSL('Documentation'))
            . '</a>';
    ...
    public static function showMySQLDocu(
        ...
        if ($just_open) {
            ...
            return $open_link
                . self::getImage('b_sqlhelp', __PMA_TRANSL('Documentation')) . '</a>';
        }
    ...
    public static function mysqlDie(
        ...
        $error_msg .= '<div class="error"><h1>' . __PMA_TRANSL('Error') . '</h1>';
        ...
        if ((!empty($sql_query)) && (!(mb_strstr($sql_query, 'connect')))) {
            ...
            if (!empty($errors)) {
                $error_msg .= '<p><strong>' . __PMA_TRANSL('Static analysis:')
                    . '</strong></p>';
                $error_msg .= '<p>' . sprintf(
                    __PMA_TRANSL('%d errors were found during analysis.'),
                    count($errors)
                ) . '</p>';
                $error_msg .= '<p><ol>';
                $error_msg .= implode(
            ...
            $error_msg .= '<p><strong>' . __PMA_TRANSL('SQL query:') . '</strong>' . "\n";
            ...
            if ($is_modify_link) {
                ...
                $error_msg .= $doedit_goto
                   . self::getIcon('b_edit', __PMA_TRANSL('Edit'))
                   . '</a>';
            }
        ...
        if (!empty($server_msg)) {
            ...
            $error_msg .= '<p>' . "\n"
                . '    <strong>' . __PMA_TRANSL('MySQL said: ') . '</strong>'
        ...
        if (!empty($back_url)) {
            ...
            $error_msg .= '<fieldset class="tblFooters">'
                . '[ <a href="' . $back_url . '">' . __PMA_TRANSL('Back') . '</a> ]'
                . '</fieldset>' . "\n\n";
        }
    ...
    public static function getMessage(
        ...
        if ($render_sql) {
                ...
                if ($is_select) {
                    ...
                    $explain_link .= ' ['
                        . self::linkOrButton(
                            ...
                            sprintf(__PMA_TRANSL('Analyze Explain at %s'), 'mariadb.org'),
                            array(),
                            '_blank'
                        ) . '&nbsp;]';
                }
            ...
            if (! empty($cfg['SQLQuery']['Edit'])
                ...
                $edit_link = ' [&nbsp;'
                    . self::linkOrButton($edit_link, __PMA_TRANSL('Edit'))
                    . '&nbsp;]';
            } else {
                $edit_link = '';
            }
            ...
            if (! empty($cfg['SQLQuery']['Refresh'])
                ...
                $refresh_link = ' [&nbsp;'
                    . self::linkOrButton($refresh_link, __PMA_TRANSL('Refresh')) . ']';
            } else {
                $refresh_link = '';
            } //refresh
    ...
    public static function formatNumber(
        ...
        /* l10n: Decimal separator */
        $decimal_sep = __PMA_TRANSL('.');
        /* l10n: Thousands separator */
        $thousands_sep = __PMA_TRANSL(',');
    ...
    public static function localisedDate($timestamp = -1, $format = '')
    {
        ...
        if ($format == '') {
            ...
            $format = __PMA_TRANSL('%B %d, %Y at %I:%M %p');
        }
    ...
    public static function timespanFormat($seconds)
    {
        ...
        return sprintf(
            __PMA_TRANSL('%s days, %s hours, %s minutes and %s seconds'),
            (string)$days,
            (string)$hours,
            (string)$minutes,
            (string)$seconds
        ...
        foreach ($params as $param) {
            if (! isset($array[$param])) {
                $error_message .= $reported_script_name
                    . ': ' . __PMA_TRANSL('Missing parameter:') . ' '
                    . $param
    ...
    public static function getListNavigator(
        ...
        if ($max_count < $count) {
            ...
            if ($frame != 'frame_navigation') {
                $list_navigator_html .= __PMA_TRANSL('Page number:');
            }
    ...
    public static function getDbLink($database = null)
    {
        ...
        return '<a href="'
            ...
            . htmlspecialchars(
                sprintf(
                    __PMA_TRANSL('Jump to database “%s”.'),
                    $database
                )
            )
    ...
    public static function getExternalBug(
        ...
        if (($component == 'mysql') && ($GLOBALS['dbi']->getVersion() < $minimum_version)) {
            $ext_but_html .= self::showHint(
                sprintf(
                    __PMA_TRANSL('The %s functionality is affected by a known bug, see %s'),
                    $functionality,
    ...
    */
    ...
    public static function getTitleForTarget($target)
    {
        $mapping = array(
            ...
            'sql' => __PMA_TRANSL('SQL'),
            'search' =>__PMA_TRANSL('Search'),
            'insert' =>__PMA_TRANSL('Insert'),
            'browse' => __PMA_TRANSL('Browse'),
            'operations' => __PMA_TRANSL('Operations'),

            // For backward compatiblity

            // Values for $cfg['DefaultTabTable']
            'tbl_structure.php' =>  __PMA_TRANSL('Structure'),
            'tbl_sql.php' => __PMA_TRANSL('SQL'),
            'tbl_select.php' =>__PMA_TRANSL('Search'),
            'tbl_change.php' =>__PMA_TRANSL('Insert'),
            'sql.php' => __PMA_TRANSL('Browse'),
            // Values for $cfg['DefaultTabDatabase']
            'db_structure.php' => __PMA_TRANSL('Structure'),
            'db_sql.php' => __PMA_TRANSL('SQL'),
            'db_search.php' => __PMA_TRANSL('Search'),
            'db_operations.php' => __PMA_TRANSL('Operations'),
        );
    ...
    public static function getBrowseUploadFileBlock($max_upload_size)
    {
        ...
        $block_html .= __PMA_TRANSL("Browse your computer:") . '</label>'
    ...
    public static function getSelectUploadFileBlock($import_list, $uploaddir)
    {
        ...
        $block_html .= '<label for="radio_local_import_file">'
            . sprintf(
                __PMA_TRANSL("Select from the web server upload directory <b>%s</b>:"),
        ...
        if ($files === false) {
            Message::error(
                __PMA_TRANSL('The directory you set for upload work cannot be reached.')
            )->display();
        } elseif (! empty($files)) {
            $block_html .= "\n"
            ...
            $block_html .= '<i>' . __PMA_TRANSL('There are no files to upload!') . '</i>';
        }
    ...
    public static function buildActionTitles()
    {
        ...
        $titles['Browse']     = self::getIcon('b_browse', __PMA_TRANSL('Browse'));
        $titles['NoBrowse']   = self::getIcon('bd_browse', __PMA_TRANSL('Browse'));
        $titles['Search']     = self::getIcon('b_select', __PMA_TRANSL('Search'));
        $titles['NoSearch']   = self::getIcon('bd_select', __PMA_TRANSL('Search'));
        $titles['Insert']     = self::getIcon('b_insrow', __PMA_TRANSL('Insert'));
        $titles['NoInsert']   = self::getIcon('bd_insrow', __PMA_TRANSL('Insert'));
        $titles['Structure']  = self::getIcon('b_props', __PMA_TRANSL('Structure'));
        $titles['Drop']       = self::getIcon('b_drop', __PMA_TRANSL('Drop'));
        $titles['NoDrop']     = self::getIcon('bd_drop', __PMA_TRANSL('Drop'));
        $titles['Empty']      = self::getIcon('b_empty', __PMA_TRANSL('Empty'));
        $titles['NoEmpty']    = self::getIcon('bd_empty', __PMA_TRANSL('Empty'));
        $titles['Edit']       = self::getIcon('b_edit', __PMA_TRANSL('Edit'));
        $titles['NoEdit']     = self::getIcon('bd_edit', __PMA_TRANSL('Edit'));
        $titles['Export']     = self::getIcon('b_export', __PMA_TRANSL('Export'));
        $titles['NoExport']   = self::getIcon('bd_export', __PMA_TRANSL('Export'));
        $titles['Execute']    = self::getIcon('b_nextpage', __PMA_TRANSL('Execute'));
        $titles['NoExecute']  = self::getIcon('bd_nextpage', __PMA_TRANSL('Execute'));
    ...
    public static function getServerSSL()
    {
        ...
        $class = 'caution';
        if (! $server['ssl']) {
            $message = __PMA_TRANSL('SSL is not being used');
            if (! empty($server['socket']) || $server['host'] == '127.0.0.1' || $server['host'] == 'localhost') {
                $class = '';
            }
        } elseif (! $server['ssl_verify']) {
            $message = __PMA_TRANSL('SSL is used with disabled verification');
        } elseif (empty($server['ssl_ca']) && empty($server['ssl_ca'])) {
            $message = __PMA_TRANSL('SSL is used without certification authority');
        } else {
            $class = '';
            $message = __PMA_TRANSL('SSL is used');
        }
    ...
    public static function getMenuTabList($level = null)
    {
        $tabList = array(
            'server' => array(
                'databases'   => __PMA_TRANSL('Databases'),
                'sql'         => __PMA_TRANSL('SQL'),
                'status'      => __PMA_TRANSL('Status'),
                'rights'      => __PMA_TRANSL('Users'),
                'export'      => __PMA_TRANSL('Export'),
                'import'      => __PMA_TRANSL('Import'),
                'settings'    => __PMA_TRANSL('Settings'),
                'binlog'      => __PMA_TRANSL('Binary log'),
                'replication' => __PMA_TRANSL('Replication'),
                'vars'        => __PMA_TRANSL('Variables'),
                'charset'     => __PMA_TRANSL('Charsets'),
                'plugins'     => __PMA_TRANSL('Plugins'),
                'engine'      => __PMA_TRANSL('Engines')
            ),
            'db'     => array(
                'structure'   => __PMA_TRANSL('Structure'),
                'sql'         => __PMA_TRANSL('SQL'),
                'search'      => __PMA_TRANSL('Search'),
                'multi_table_query'         => __PMA_TRANSL('Query'),
                'export'      => __PMA_TRANSL('Export'),
                'import'      => __PMA_TRANSL('Import'),
                'operation'   => __PMA_TRANSL('Operations'),
                'privileges'  => __PMA_TRANSL('Privileges'),
                'routines'    => __PMA_TRANSL('Routines'),
                'events'      => __PMA_TRANSL('Events'),
                'triggers'    => __PMA_TRANSL('Triggers'),
                'tracking'    => __PMA_TRANSL('Tracking'),
                'designer'    => __PMA_TRANSL('Designer'),
                'central_columns' => __PMA_TRANSL('Central columns')
            ),
            'table'  => array(
                'browse'      => __PMA_TRANSL('Browse'),
                'structure'   => __PMA_TRANSL('Structure'),
    ...
    public static function sortableTableHeader($title, $sort, $initialSortOrder = 'ASC')
    {
        ...
        $orderLinkParams['title'] = __PMA_TRANSL('Sort');
        ...
        if ($requestedSort == $sort) {
            if ($requestedSortOrder == 'ASC') {
                ...
                $orderImg = ' ' . self::getImage(
                    's_asc',
                    __PMA_TRANSL('Ascending'),
                    array('class' => 'sort_arrow', 'title' => '')
                );
                $orderImg .= ' ' . self::getImage(
                    's_desc',
                     __PMA_TRANSL('Descending'),
