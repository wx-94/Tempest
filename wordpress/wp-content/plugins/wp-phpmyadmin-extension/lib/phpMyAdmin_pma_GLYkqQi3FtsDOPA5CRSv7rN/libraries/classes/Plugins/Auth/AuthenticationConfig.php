...
class AuthenticationConfig extends AuthenticationPlugin
{
    ...
    public function showFailure($failure)
    {
        ...
        if (!$conn_error) {
            $conn_error = __PMA_TRANSL('Cannot connect: invalid settings.');
        }
        ...
        $header->setTitle(__PMA_TRANSL('Access denied!'));
        $header->disableMenuAndConsole();
        echo '<br /><br />
    <center>
        <h1>';
        echo sprintf(__PMA_TRANSL('Welcome to %s'), ' phpMyAdmin ');
        echo '</h1>
    </center>
    <br />
    <table cellpadding="0" cellspacing="3" class= "auth_config_tbl" width="80%">
        ...
        if (isset($GLOBALS['allowDeny_forbidden'])
            ...
            trigger_error(__PMA_TRANSL('Access denied!'), E_USER_NOTICE);
        } else {
