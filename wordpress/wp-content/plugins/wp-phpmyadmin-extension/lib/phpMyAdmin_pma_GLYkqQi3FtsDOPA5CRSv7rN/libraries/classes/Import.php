...
class Import
{
    ...
    public static function executeQuery($sql, $full, array &$sql_data)
    {
        ...
        if ($result === false) { // execution failed
            ...
            if ($a_num_rows > 0) {
                $msg .= __PMA_TRANSL('Rows') . ': ' . $a_num_rows;
            } elseif ($a_aff_rows > 0) {
    ...
    public static function runQuery($sql = '', $full = '', array &$sql_data = array())
    {
        ...
        if (isset($_REQUEST['rollback_query'])) {
            $msg .= __PMA_TRANSL('[ROLLBACK occurred.]');
        }
    }
    ...
    public static function buildSql($db_name, array &$tables, &$analyses = null,
        ...
        $message = '<br /><br />';
        $message .= '<strong>' . __PMA_TRANSL(
            'The following structures have either been created or altered. Here you can:'
        ) . '</strong><br />';
        $message .= '<ul><li>' . __PMA_TRANSL(
            "View a structure's contents by clicking on its name."
        ) . '</li>';
        $message .= '<li>' . __PMA_TRANSL(
            'Change any of its settings by clicking the corresponding "Options" link.'
        ) . '</li>';
        $message .= '<li>' . __PMA_TRANSL('Edit structure by following the "Structure" link.')
            . '</li>';
        $message .= sprintf(
            '<br /><li><a href="%s" title="%s">%s</a> (<a href="%s" title="%s">'
            . __PMA_TRANSL('Options') . '</a>)</li>',
            $db_url,
            sprintf(
                __PMA_TRANSL('Go to database: %s'),
                htmlspecialchars(Util::backquote($db_name))
            ),
            htmlspecialchars($db_name),
            $db_ops_url,
            sprintf(
                __PMA_TRANSL('Edit settings for %s'),
                htmlspecialchars(Util::backquote($db_name))
            )
        );
        ...
        for ($i = 0; $i < $num_tables; ++$i) {
            ...
            if (! $_table->isView()) {
                $message .= sprintf(
                    '<li><a href="%s" title="%s">%s</a> (<a href="%s" title="%s">' . __PMA_TRANSL(
                        'Structure'
                    ) . '</a>) (<a href="%s" title="%s">' . __PMA_TRANSL('Options') . '</a>)</li>',
                    $tbl_url,
                    sprintf(
                        __PMA_TRANSL('Go to table: %s'),
                        htmlspecialchars(
                            Util::backquote($tables[$i][self::TBL_NAME])
                        )
                    ),
    ...
    public static function handleSimulateDmlRequest()
    {
        $response = Response::getInstance();
        $error = false;
        $error_msg = __PMA_TRANSL('Only single-table UPDATE and DELETE queries can be simulated.');
