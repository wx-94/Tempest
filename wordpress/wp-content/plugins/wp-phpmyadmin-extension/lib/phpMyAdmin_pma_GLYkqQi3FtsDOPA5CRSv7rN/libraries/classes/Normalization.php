...
class Normalization
{
    ...
    {
        $step = 1;
        $stepTxt = __PMA_TRANSL('Make all columns atomic');
        $html = "<h3 class='center'>"
            . __PMA_TRANSL('First step of normalization (1NF)') . "</h3>";
        $html .= "<div id='mainContent' data-normalizeto='" . $normalizedTo . "'>" .
            "<fieldset>" .
            "<legend>" . __PMA_TRANSL('Step 1.') . $step . " " . $stepTxt . "</legend>" .
            "<h4>" . __PMA_TRANSL(
            ...
            . __PMA_TRANSL('Select one…') . "</option>"
            . "<option value='no_such_col'>" . __PMA_TRANSL('No such column') . "</option>"
            ...
            . "</select>"
            . "<span>" . __PMA_TRANSL('split into ')
            . "</span><input id='numField' type='number' value='2'>"
            . "<input type='submit' id='splitGo' value='" . __PMA_TRANSL('Go') . "'/></div>"
            . "<div id='newCols'></div>"
    ...
    {
        $step = 2;
        $stepTxt = __PMA_TRANSL('Have a primary key');
        $primary = Index::getPrimary($table, $db);
        $hasPrimaryKey = "0";
        $legendText = __PMA_TRANSL('Step 1.') . $step . " " . $stepTxt;
        $extra = '';
        if ($primary) {
            $headText = __PMA_TRANSL("Primary key already exists.");
            $subText = __PMA_TRANSL("Taking you to next step…");
            $hasPrimaryKey = "1";
        } else {
            $headText = __PMA_TRANSL(
            ...
            $extra = __PMA_TRANSL(
                ...
                . __PMA_TRANSL('+ Add a new primary key column') . '</a>';
        }
        $res = [
    ...
    {
        $step = 4;
        $stepTxt = __PMA_TRANSL('Remove redundant columns');
        $legendText = __PMA_TRANSL('Step 1.') . $step . " " . $stepTxt;
        $headText = __PMA_TRANSL(
        ...
        $extra = $this->getHtmlForColumnsList($db, $table, 'all', "checkbox") . "</br>"
            ...
            . __PMA_TRANSL('Remove selected') . '"/>'
            . '<input type="submit" value="' . __PMA_TRANSL('No redundant column')
    ...
    {
        $step = 3;
        $stepTxt = __PMA_TRANSL('Move repeating groups');
        $legendText = __PMA_TRANSL('Step 1.') . $step . " " . $stepTxt;
        $headText = __PMA_TRANSL(
        ...
        $extra = $this->getHtmlForColumnsList($db, $table, 'all', "checkbox") . "</br>"
            ...
            . __PMA_TRANSL('Done') . '"/>'
            . '<input type="submit" value="' . __PMA_TRANSL('No repeating group')
            . '" onclick="goToStep4();"'
            . '/>';
        $primary = Index::getPrimary($table, $db);
    ...
    {
        $legendText = __PMA_TRANSL('Step 2.') . "1 " . __PMA_TRANSL('Find partial dependencies');
        $primary = Index::getPrimary($table, $db);
        $primarycols = $primary->getColumns();
        $pk = [];
        $subText = '';
        ...
        if (count($primarycols) > 1) {
            ...
            if (count($pk) == count($columns)) {
                ...
                $extra = '<h3>' . __PMA_TRANSL('Table is already in second normal form.')
                    . '</h3>';
            } else {
                $headText = sprintf(
                    __PMA_TRANSL(
                    ...
                    . '<br/><a href="#" id="showPossiblePd">' . __PMA_TRANSL(
            ...
            $extra = '<h3>' . __PMA_TRANSL('Table is already in second normal form.') . '</h3>';
        }
        $res = [
            'legendText' => $legendText,
            'headText' => $headText,
    ...
    public function createNewTablesFor2NF(array $partialDependencies, $tablesName, $table, $db)
    {
        ...
        $error = false;
        $headText = '<h3>' . sprintf(
            __PMA_TRANSL('The second step of normalization is complete for table \'%1$s\'.'),
            htmlspecialchars($table)
        ) . '</h3>';
        if (count((array)$partialDependencies) == 1) {
            return [
                'legendText'=>__PMA_TRANSL('End of step'), 'headText'=>$headText,
                'queryError'=>$error
            ];
        }
        $message = '';
        ...
        foreach ($queries as $query) {
            if (!$this->dbi->tryQuery($query)) {
                $message = Message::error(__PMA_TRANSL('Error in processing!'));
                $message->addMessage(
                    Message::rawError(
                        $this->dbi->getError()
                    ),
        ...
        return [
            'legendText' => __PMA_TRANSL('End of step'),
            'headText' => $headText,
            'queryError' => $error,
            'extra' => $message
        ];
    ...
    public function createNewTablesFor3NF(array $newTables, $db)
    {
        ...
        $error = false;
        $headText = '<h3>' .
            __PMA_TRANSL('The third step of normalization is complete.')
            . '</h3>';
    ...
    public function moveRepeatingGroup(
        ...
        $message = Message::success(
            sprintf(
                __PMA_TRANSL('Selected repeating group has been moved to the table \'%s\''),
                htmlspecialchars($table)
            )
        );
        $first = true;
    ...
    {
        $legendText = __PMA_TRANSL('Step 3.') . "1 " . __PMA_TRANSL('Find transitive dependencies');
        $extra = "";
        $headText = __PMA_TRANSL(
            'Please answer the following question(s) '
            . 'carefully to obtain a correct normalization.'
        ...
        if ($extra == "") {
            ...
            $subText = "";
            $extra = "<h3>" . __PMA_TRANSL("Table is already in Third normal form!") . "</h3>";
        }
        $res = [
            'legendText' => $legendText,
            'headText' => $headText,
    ...
    public function getHtmlForNormalizeTable()
    {
        ...
        $htmlOutput .= '<fieldset>';
        $htmlOutput .= '<legend>'
            . __PMA_TRANSL('Improve table structure (Normalization):') . '</legend>';
        $htmlOutput .= '<h3>' . __PMA_TRANSL('Select up to what step you want to normalize')
            . '</h3>';
        $choices = [
                '1nf' => __PMA_TRANSL('First step of normalization (1NF)'),
                '2nf'      => __PMA_TRANSL('Second step of normalization (1NF+2NF)'),
                '3nf'  => __PMA_TRANSL('Third step of normalization (1NF+2NF+3NF)')];
        ...
        $htmlOutput .= '</fieldset><fieldset class="tblFooters">'
            . "<span class='floatleft'>" . __PMA_TRANSL(
                'Hint: Please follow the procedure carefully in order '
                . 'to obtain correct normalization'
            ) . "</span>"
            . '<input type="submit" name="submit_normalize" value="' . __PMA_TRANSL('Go') . '" />'
            . '</fieldset>'
            . '</form>'
            . '</div>';
    ...
    public function findPartialDependencies($table, $db)
    {
        ...
        foreach ($dependencyList as $dependon => $colList) {
            $html .= '<span class="displayblock">'
                . '<input type="button" class="pickPd" value="' . __PMA_TRANSL('Pick') . '"/>'
        ...
        if (empty($dependencyList)) {
            $html .= '<p class="displayblock desc">'
                . __PMA_TRANSL('No partial dependencies found!') . '</p>';
        }
        $html .= '</div>';
        return $html;
    }
