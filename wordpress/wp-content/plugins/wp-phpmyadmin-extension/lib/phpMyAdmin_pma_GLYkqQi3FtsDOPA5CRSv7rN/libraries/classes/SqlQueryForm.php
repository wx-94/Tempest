...
class SqlQueryForm
{
    ...
    public static function init($query)
    {
        ...
        if (strlen($GLOBALS['db']) === 0) {
            // prepare for server related
            $legend = sprintf(
                __PMA_TRANSL('Run SQL query/queries on server “%s”'),
                htmlspecialchars(
            ...
            $legend = sprintf(__PMA_TRANSL('Run SQL query/queries on database %s'), $tmp_db_link);
            if (empty($query)) {
            ...
            $legend = sprintf(__PMA_TRANSL('Run SQL query/queries on table %s'), $tmp_tbl_link);
            if (empty($query)) {
                $query = Util::expandUserString(
    ...
    public static function getHtmlForInsert(
        ...
        $html .= '<input type="button" value="' . __PMA_TRANSL('Clear') . '" id="clear"'
            . ' class="button sqlbutton" />';
        if ($GLOBALS['cfg']['CodemirrorEnable']) {
            $html .= '<input type="button" value="' . __PMA_TRANSL('Format') . '" id="format"'
                . ' class="button sqlbutton" />';
        }
        $html .= '<input type="button" value="' . __PMA_TRANSL('Get auto-saved query')
            . '" id="saved" class="button sqlbutton" />';

        // parameter binding
        $html .= '<div>';
        $html .= '<input type="checkbox" name="parameterized" id="parameterized" />';
        $html .= '<label for="parameterized">' . __PMA_TRANSL('Bind parameters') . '</label>';
        ...
        if (! empty($columns_list)) {
            $html .= '<div id="tablefieldscontainer">'
                . '<label>' . __PMA_TRANSL('Columns') . '</label>'
            ...
            if (Util::showIcons('ActionLinksMode')) {
                $html .= '<input type="button" class="button" name="insert"'
                    ...
                    . ' title="' . __PMA_TRANSL('Insert') . '" />';
            } else {
                $html .= '<input type="button" class="button" name="insert"'
                    . ' value="' . __PMA_TRANSL('Insert') . '"'
        ...
        if ($cfgBookmark) {
            ...
            $html .= '<label for="bkm_label">'
                . __PMA_TRANSL('Bookmark this SQL query:') . '</label>';
            ...
            $html .= '<label for="id_bkm_all_users">'
                . __PMA_TRANSL('Let every user access this bookmark') . '</label>';
            $html .= '</div>';
            $html .= '<div class="formelement">';
            $html .= '<input type="checkbox" name="bkm_replace" tabindex="112"'
                . ' id="id_bkm_replace" value="true" />';
            $html .= '<label for="id_bkm_replace">'
                . __PMA_TRANSL('Replace existing bookmark of same name') . '</label>';
            $html .= '</div>';
            $html .= '</div>';
        }
        ...
        $html .= '<div class="formelement">';
        $html .= '<label for="id_sql_delimiter">[ ' . __PMA_TRANSL('Delimiter')
            . '</label>' . "\n";
        ...
        $html .= '<input type="checkbox" name="show_query" value="1" '
            ...
            . '<label for="checkbox_show_query">' . __PMA_TRANSL('Show this query here again')
            . '</label>';
        $html .= '</div>';
        ...
        $html .= '<input type="checkbox" name="retain_query_box" value="1" '
            ...
            . ($GLOBALS['cfg']['RetainQueryBox'] === false
                ? '' : ' checked="checked"')
            . ' />'
            . '<label for="retain_query_box">' . __PMA_TRANSL('Retain query box')
            . '</label>';
        $html .= '</div>';
        ...
        $html .= '<input type="checkbox" name="rollback_query" value="1" '
            ...
            . '<label for="rollback_query">' . __PMA_TRANSL('Rollback when finished')
            . '</label>';
        $html .= '</div>';
        ...
        $html .= ' tabindex="200" value="' . __PMA_TRANSL('Go') . '" />' . "\n";
        $html .= '<div class="clearfloat"></div>' . "\n";
        $html .= '</fieldset>' . "\n";
    ...
    public static function getHtmlForBookmark()
    {
        ...
        $html .= '<legend>';
        $html .= __PMA_TRANSL('Bookmarked SQL query') . '</legend>' . "\n";
        $html .= '<div class="formelement">';
        ...
        foreach ($bookmark_list as $bookmark) {
            $html .= '<option value="' . htmlspecialchars($bookmark->getId()) . '"'
                ...
                . (empty($bookmark->getUser()) ? (' (' . __PMA_TRANSL('shared') . ')') : '')
                . '</option>' . "\n";
        }
        ...
        $html .= '<input type="radio" name="action_bookmark" value="0"'
            ...
            . '<label for="radio_bookmark_exe">' . __PMA_TRANSL('Submit')
            . '</label>' . "\n";
        $html .= '<input type="radio" name="action_bookmark" value="1"'
            . ' id="radio_bookmark_view" />'
            . '<label for="radio_bookmark_view">' . __PMA_TRANSL('View only')
            . '</label>' . "\n";
        $html .= '<input type="radio" name="action_bookmark" value="2"'
            . ' id="radio_bookmark_del" />'
            . '<label for="radio_bookmark_del">' . __PMA_TRANSL('Delete')
            . '</label>' . "\n";
        $html .= '</div>' . "\n";
        $html .= '<div class="clearfloat"></div>' . "\n";
        $html .= '<div class="formelement hide">' . "\n";
        $html .= __PMA_TRANSL('Variables');
