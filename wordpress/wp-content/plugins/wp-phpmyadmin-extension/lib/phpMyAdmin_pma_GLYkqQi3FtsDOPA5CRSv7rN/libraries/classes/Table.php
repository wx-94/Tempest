...
class Table
{
    ...
    public function getStatusInfo(
        ...
        if (!array_key_exists(
            ...
            if (! $disable_error) {
                trigger_error(
                    __PMA_TRANSL('Unknown table status:') . ' ' . $info,
                    E_USER_WARNING
                );
            }
            return false;
        ...
        if (! $GLOBALS['dblist']->databases->exists($source_db, $target_db)) {
            if (! $GLOBALS['dblist']->databases->exists($source_db)) {
                $GLOBALS['message'] = Message::rawError(
                    sprintf(
                        __PMA_TRANSL('Source database `%s` was not found!'),
            ...
            if (! $GLOBALS['dblist']->databases->exists($target_db)) {
                $GLOBALS['message'] = Message::rawError(
                    sprintf(
                        __PMA_TRANSL('Target database `%s` was not found!'),
    ...
    public function rename($new_name, $new_db = null)
    {
        ...
        if (null !== $new_db && $new_db !== $this->getDbName()) {
            ...
            if (! $GLOBALS['dblist']->databases->exists($new_db)) {
                $this->errors[] = __PMA_TRANSL('Invalid database:') . ' ' . $new_db;
                return false;
            }
        } else {
        ...
        if (! self::isValidName($new_name, true)) {
            $this->errors[] = __PMA_TRANSL('Invalid table name:') . ' '
        ...
        if (! $this->_dbi->query($GLOBALS['sql_query'])) {
            ...
            $this->errors[] = sprintf(
                __PMA_TRANSL('Failed to rename table %1$s to %2$s!'),
                $this->getFullName(),
                $new_table->getFullName()
            );
            return false;
        ...
        $this->messages[] = sprintf(
            __PMA_TRANSL('Table %1$s has been renamed to %2$s.'),
    ...
    protected function saveUiPrefsToDb()
    {
        ...
        if (!$success) {
            $message = Message::error(
                __PMA_TRANSL('Could not save table UI preferences!')
            );
            $message->addMessage(
                Message::rawError(
    ...
    public function getSqlQueryForIndexCreateOrEdit($index, &$error)
    {
        ...
        case 'PRIMARY':
            if ($index->getName() == '') {
                ...
                $error = Message::error(
                    __PMA_TRANSL('The name of the primary key must be "PRIMARY"!')
                );
            }
        ...
        case 'SPATIAL':
            if ($index->getName() == 'PRIMARY') {
                $error = Message::error(
                    __PMA_TRANSL('Can\'t rename index to PRIMARY!')
                );
            }
            $sql_query .= sprintf(
                ' ADD %s ',
        ...
        if (empty($index_fields)) {
            $error = Message::error(__PMA_TRANSL('No index parts defined!'));
        } else {
