        ...
        */
        ...
        if ($res === true) {
            if ($this->zip->numFiles === 0) {
                $error_message = __PMA_TRANSL('No files found inside ZIP archive!');
                $this->zip->close();
            ...
            if (empty($file_data)) {
                $error_message = __PMA_TRANSL('Error in ZIP archive:')
            ...
            $error_message = __PMA_TRANSL('Error in ZIP archive:') . ' ' . $this->zip->getStatusString();
            $this->zip->close();
            return (['error' => $error_message, 'data' => $file_data]);
        }
    }
