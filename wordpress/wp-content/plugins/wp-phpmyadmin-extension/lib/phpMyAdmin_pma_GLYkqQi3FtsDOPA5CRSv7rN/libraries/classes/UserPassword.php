...
class UserPassword
{
    ...
    {
        $error = false;
        $message = Message::success(__PMA_TRANSL('The profile has been updated.'));
        ...
        if (($_REQUEST['nopass'] != '1')) {
            if (strlen($_REQUEST['pma_pw']) === 0 || strlen($_REQUEST['pma_pw2']) === 0) {
                ...
                $message = Message::error(
                    __PMA_TRANSL('The passwords aren\'t the same!')
                );
                $error = true;
            } elseif (strlen($_REQUEST['pma_pw']) > 256) {
                $message = Message::error(__PMA_TRANSL('Password is too long!'));
                $error = true;
            }
        }
