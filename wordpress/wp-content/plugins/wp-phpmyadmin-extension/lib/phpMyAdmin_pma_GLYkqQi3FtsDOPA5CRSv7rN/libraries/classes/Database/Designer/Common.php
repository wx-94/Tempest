...
class Common
{
    ...
    public function addNewRelation($db, $T1, $F1, $T2, $F2, $on_delete, $on_update, $DB1, $DB2)
    {
        ...
        if (Util::isForeignKeySupported($type_T1)
            ...
            if ($foreigner
                ...
                return array(false, __PMA_TRANSL('Error: relationship already exists.'));
            }
            ...
            if (! empty($index_array1[$F1]) && ! empty($index_array2[$F2])) {
                ...
                if ($GLOBALS['dbi']->tryQuery($upd_query)) {
                    return array(true, __PMA_TRANSL('FOREIGN KEY relationship has been added.'));
                }

                $error = $GLOBALS['dbi']->getError();
                return array(
                    false,
                    __PMA_TRANSL('Error: FOREIGN KEY relationship could not be added!')
                    . "<br/>" . $error
                );
            }

            return array(false, __PMA_TRANSL('Error: Missing index on column(s).'));
        }

        // internal (pmadb) relation
        if ($GLOBALS['cfgRelation']['relwork'] == false) {
            return array(false, __PMA_TRANSL('Error: Relational features are disabled!'));
        }
        ...
        if ($this->relation->queryAsControlUser($q, false, DatabaseInterface::QUERY_STORE)
            ...
            return array(true, __PMA_TRANSL('Internal relationship has been added.'));
        }

        $error = $GLOBALS['dbi']->getError(DatabaseInterface::CONNECT_CONTROL);
        return array(
            false,
            __PMA_TRANSL('Error: Internal relationship could not be added!')
            . "<br/>" . $error
        );
    }
    ...
    public function removeRelation($T1, $F1, $T2, $F2)
    {
        ...
        if (Util::isForeignKeySupported($type_T1)
            ...
            if (isset($foreigner['constraint'])) {
                ...
                if ($GLOBALS['dbi']->query($upd_query)) {
                    return array(true, __PMA_TRANSL('FOREIGN KEY relationship has been removed.'));
                }

                $error = $GLOBALS['dbi']->getError();
                return array(
                    false,
                    __PMA_TRANSL('Error: FOREIGN KEY relationship could not be removed!')
                    . "<br/>" . $error
                );
            }
        }
        ...
        if (!$result) {
            ...
            return array(
                false,
                __PMA_TRANSL('Error: Internal relationship could not be removed!') . "<br/>" . $error
            );
        }

        return array(true, __PMA_TRANSL('Internal relationship has been removed.'));
    }
