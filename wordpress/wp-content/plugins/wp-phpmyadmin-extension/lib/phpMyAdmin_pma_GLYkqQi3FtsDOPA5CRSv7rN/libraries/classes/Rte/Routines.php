...
class Routines
{
    ...
    public static function handleEditor()
    {
        ...
        if (count($errors)
            ...
            if (! empty($_REQUEST['add_item'])) {
                ...
                $title = __PMA_TRANSL("Edit routine");
            ...
            if ($routine !== false) {
                ...
                $message  = __PMA_TRANSL('Error in processing request:') . ' ';
                $message .= sprintf(
                    Words::get('no_edit'),
                    htmlspecialchars(
    ...
    public static function handleRequestCreateOrEdit(array $errors, $db)
    {
        ...
        if (!count($errors)) { // set by self::getQueryFromRequest()
            ...
            if (!empty($_REQUEST['editor_process_edit'])) {
                ...
                if (!$isProcOrFunc) {
                    $errors[] = sprintf(
                        __PMA_TRANSL('Invalid routine type: "%s"'),
                    ...
                    if (!$result) {
                        $errors[] = sprintf(
                            __PMA_TRANSL('The following query has failed: "%s"'),
                            htmlspecialchars($drop_routine)
                        )
                        . '<br />'
                        . __PMA_TRANSL('MySQL said: ') . $GLOBALS['dbi']->getError(null);
                    } else {
                ...
                if (!$result) {
                    ...
                    $message = Message::success(
                        __PMA_TRANSL('Routine %1$s has been created.')
                    );
                    $message->addParam(
                        Util::backquote($_REQUEST['item_name'])
                    );
    ...
    public static function flushPrivileges($flushPrivileges)
    {
        if ($flushPrivileges) {
            ...
            $message = Message::success(
                __PMA_TRANSL('Routine %1$s has been modified.')
            );
        }
        $message->addParam(
            Util::backquote($_REQUEST['item_name'])
    ...
    public static function getParameterRow(array $routine = array(), $index = null, $class = '')
    {
        ...
        $retval .= "                        "
            . Util::getImage('b_edit', '', array('title'=>__PMA_TRANSL('ENUM/SET editor')))
            . "\n";
        $retval .= "                    </a>\n";
        $retval .= "                </div>\n";
        $retval .= "            </td>\n";
    ...
    public static function getEditorForm($mode, $operation, array $routine)
    {
        ...
        $retval .= "<fieldset>\n";
        $retval .= "<legend>" . __PMA_TRANSL('Details') . "</legend>\n";
        $retval .= "<table class='rte_table' style='width: 100%'>\n";
        $retval .= "<tr>\n";
        $retval .= "    <td style='width: 20%;'>" . __PMA_TRANSL('Routine name') . "</td>\n";
        $retval .= "    <td><input type='text' name='item_name' maxlength='64'\n";
        $retval .= "               value='{$routine['item_name']}' /></td>\n";
        $retval .= "</tr>\n";
        $retval .= "<tr>\n";
        $retval .= "    <td>" . __PMA_TRANSL('Type') . "</td>\n";
        $retval .= "    <td>\n";
        if ($response->isAjax()) {
            ...
            $retval .= "<input name='item_type' type='hidden'"
                ...
                . " value='" . sprintf(__PMA_TRANSL('Change to %s'), $routine['item_type_toggle'])
                . "' />\n";
        }
        $retval .= "    </td>\n";
        $retval .= "</tr>\n";
        $retval .= "<tr>\n";
        $retval .= "    <td>" . __PMA_TRANSL('Parameters') . "</td>\n";
        $retval .= "    <td>\n";
        ...
        $retval .= "            <th class='routine_direction_cell$isprocedure_class'>"
            . __PMA_TRANSL('Direction') . "</th>\n";
        $retval .= "            <th>" . __PMA_TRANSL('Name') . "</th>\n";
        $retval .= "            <th>" . __PMA_TRANSL('Type') . "</th>\n";
        $retval .= "            <th>" . __PMA_TRANSL('Length/Values') . "</th>\n";
        $retval .= "            <th colspan='2'>" . __PMA_TRANSL('Options') . "</th>\n";
        ...
        if (isset($_REQUEST['edit_item'])
            ...
            $retval .= "<tr>";
            $retval .= "    <td>" . __PMA_TRANSL('Adjust privileges');
            ...
            if ($GLOBALS['proc_priv']
                ...
                $retval .= "    <td><input type='checkbox' "
                    . "name='item_adjust_privileges' value='1' title='" . __PMA_TRANSL(
        ...
        $retval .= "<tr>";
        $retval .= "    <td>" . __PMA_TRANSL('Definer') . "</td>";
        $retval .= "    <td><input type='text' name='item_definer'";
        $retval .= "               value='" . $routine['item_definer'] . "' /></td>";
        $retval .= "</tr>";
        $retval .= "<tr>";
        $retval .= "    <td>" . __PMA_TRANSL('Security type') . "</td>";
        ...
        $retval .= "</tr>";
        $retval .= "<tr>";
        $retval .= "    <td>" . __PMA_TRANSL('SQL data access') . "</td>";
        ...
        $retval .= "</tr>";
        $retval .= "<tr>";
        $retval .= "    <td>" . __PMA_TRANSL('Comment') . "</td>";
    ...
    public static function getQueryFromRequest()
    {
        ...
        if (! empty($_REQUEST['item_definer'])) {
            if (mb_strpos($_REQUEST['item_definer'], '@') !== false) {
                ...
                $errors[] = __PMA_TRANSL('The definer must be in the "username@hostname" format!');
            }
        }
        ...
        if (! empty($_REQUEST['item_name'])) {
            ...
            $errors[] = __PMA_TRANSL('You must provide a routine name!');
        }
        $params = '';
            ...
            for ($i=0, $nb = count($item_param_name); $i < $nb; $i++) {
                if (! empty($item_param_name[$i])
                    ...
                    if ($_REQUEST['item_type'] == 'PROCEDURE'
                        ...
                        $errors[] = sprintf(
                            __PMA_TRANSL('Invalid direction "%s" given for parameter.'),
        ...
        if ($_REQUEST['item_type'] == 'FUNCTION') {
            ...
            if (! empty($item_returntype)
                ...
                $errors[] = __PMA_TRANSL('You must provide a valid return type for the routine.');
            }
        ...
        if (! empty($_REQUEST['item_definition'])) {
            ...
            $errors[] = __PMA_TRANSL('You must provide a routine definition.');
        }
    ...
    public static function handleExecute()
    {
        ...
        if (! empty($_REQUEST['execute_routine']) && ! empty($_REQUEST['item_name'])) {
            ...
            if ($outcome) {
                ...
                $output .= sprintf(
                    __PMA_TRANSL('Execution results of routine %s'),
            ...
            if ($outcome) {
                ...
                $message = __PMA_TRANSL('Your SQL query has been executed successfully.');
                if ($routine['item_type'] == 'PROCEDURE') {
                    $message .= '<br />';
            ...
            if ($routine !== false) {
                ...
                if ($response->isAjax()) {
                    $title = __PMA_TRANSL("Execute routine") . " " . Util::backquote(
                    ...
                    echo "\n\n<h2>" . __PMA_TRANSL("Execute routine") . "</h2>\n\n";
                    echo $form;
                }
                exit;
    ...
    public static function getExecuteForm(array $routine)
    {
        ...
        if (! $response->isAjax()) {
            ...
            $retval .= __PMA_TRANSL('Routine parameters');
            $retval .= "</caption>\n";
        } else {
            $retval .= "<legend>" . __PMA_TRANSL('Routine parameters') . "</legend>\n";
            $retval .= "<table class='rte_table' style='width: 100%;'>\n";
        }
        $retval .= "<tr>\n";
        $retval .= "<th>" . __PMA_TRANSL('Name') . "</th>\n";
        ...
        if ($cfg['ShowFunctionFields']) {
            $retval .= "<th>" . __PMA_TRANSL('Function') . "</th>\n";
        }
