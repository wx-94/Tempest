...
class Export
{
    ...
    {
        $trans = new Message;
        $trans->addText(__PMA_TRANSL('@SERVER@ will become the server name'));
        if ($exportType == 'database' || $exportType == 'table') {
            $trans->addText(__PMA_TRANSL(', @DATABASE@ will become the database name'));
            if ($exportType == 'table') {
                $trans->addText(__PMA_TRANSL(', @TABLE@ will become the table name'));
            }
        }
        ...
        $msg->addParamHtml(
            ...
            . '" target="documentation" title="' . __PMA_TRANSL('Documentation') . '">'
        );
        $msg->addParamHtml('</a>');
        $msg->addParam($trans);
    ...
    {
        $title = __PMA_TRANSL('Rename exported databases/tables/columns');
    ...
    public function getDisplay(
        ...
        if (empty($exportList)) {
            Message::error(
                __PMA_TRANSL('Could not load export plugins, please check your installation!')
            )->display();
            exit;
        }
