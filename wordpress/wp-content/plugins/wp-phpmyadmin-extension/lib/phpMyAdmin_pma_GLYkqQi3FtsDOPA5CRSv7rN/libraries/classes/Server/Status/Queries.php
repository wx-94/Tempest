...
class Queries
{
    ...
    public static function getHtmlForQueryStatistics(Data $serverStatusData)
    {
        ...
        $retval .= sprintf(
            __PMA_TRANSL('Questions since startup: %s'),
        ...
        $retval .= '<br />';
        $retval .= '<span>';
        $retval .= '&oslash; ' . __PMA_TRANSL('per hour:') . ' ';
        $retval .= Util::formatNumber($total_queries * $hour_factor, 0);
        $retval .= '<br />';
        $retval .= '&oslash; ' . __PMA_TRANSL('per minute:') . ' ';
        ...
        if ($total_queries / $serverStatusData->status['Uptime'] >= 1) {
            $retval .= '&oslash; ' . __PMA_TRANSL('per second:') . ' ';
            $retval .= Util::formatNumber(
    ...
    public static function getHtmlForDetails(Data $serverStatusData)
    {
        ...
        $retval .= '<thead>';
        $retval .= '<tr><th>' . __PMA_TRANSL('Statements') . '</th>';
        $retval .= '<th>';
        /* l10n: # = Amount of queries */
        $retval .= __PMA_TRANSL('#');
        $retval .= '</th>';
        $retval .= '<th>&oslash; ' . __PMA_TRANSL('per hour')
            . '</th>';
        $retval .= '<th>%</div></th>';
        $retval .= '</tr>';
        $retval .= '</thead>';
        ...
        if ($other_sum > 0) {
            $chart_json[__PMA_TRANSL('Other')] = $other_sum;
        }
