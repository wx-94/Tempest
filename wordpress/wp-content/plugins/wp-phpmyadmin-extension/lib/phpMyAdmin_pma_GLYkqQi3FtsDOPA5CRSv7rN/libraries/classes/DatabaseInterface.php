...
class DatabaseInterface
{
    ...
    */
    ...
    public function setCollation($collation)
    {
        ...
        if ($result === false) {
            trigger_error(
                __PMA_TRANSL('Failed to set configured collation connection!'),
                E_USER_WARNING
            );
        } else {
    ...
    public static function formatError($error_number, $error_message)
    {
        ...
        if ($error_number == 2002) {
            ...
            $error .= ' - ' . $error_message;
            $error .= $separator . __PMA_TRANSL('The server is not responding.');
        } elseif ($error_number == 1698 ) {
            $error .= ' - ' . $error_message;
            $error .= $separator . '<a href="logout.php' . Url::getCommon() . '">';
            $error .= __PMA_TRANSL('Logout and try as another user.') . '</a>';
    ...
    public function connect($mode, $server = null, $target = null)
    {
        ...
        if (is_null($user) || is_null($password)) {
            trigger_error(
                __PMA_TRANSL('Missing connection parameters!'),
                E_USER_WARNING
            );
            return false;
        }
    ...
    public static function load()
    {
        if (defined('TESTSUITE')) {
            ...
            if (! self::checkDbExtension($extension)) {
                ...
                $doclink = sprintf(
                    __PMA_TRANSL('See %sour documentation%s for more information.'),
