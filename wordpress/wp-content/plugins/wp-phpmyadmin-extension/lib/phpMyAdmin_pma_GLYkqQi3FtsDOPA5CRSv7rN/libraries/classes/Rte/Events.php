...
class Events
{
            ...
            if (! count($errors)) { // set by PhpMyAdmin\Rte\Routines::getQueryFromRequest()
                ...
                if (! empty($_REQUEST['editor_process_edit'])) {
                    ...
                    if (! $result) {
                        ...
                        if (! $result) {
                            ...
                            $message = Message::success(
                                __PMA_TRANSL('Event %1$s has been modified.')
                            );
                            $message->addParam(
                    ...
                    if (! $result) {
                        ...
                        $message = Message::success(
                            __PMA_TRANSL('Event %1$s has been created.')
                        );
                        $message->addParam(
                            Util::backquote($_REQUEST['item_name'])
                        );
    ...
    public static function getEditorForm($mode, $operation, array $item)
    {
        ...
        $retval .= "<tr>\n";
        $retval .= "    <td>" . __PMA_TRANSL('Event type') . "</td>\n";
        $retval .= "    <td>\n";
        if ($response->isAjax()) {
            ...
            $retval .= "               value='";
            $retval .= sprintf(__PMA_TRANSL('Change to %s'), $item['item_type_toggle']);
            $retval .= "' />\n";
        }
        $retval .= "    </td>\n";
        $retval .= "</tr>\n";
        $retval .= "<tr class='onetime_event_row $isonetime_class'>\n";
        $retval .= "    <td>" . __PMA_TRANSL('Execute at') . "</td>\n";
        ...
        $retval .= "    <td>" . __PMA_TRANSL('Execute every') . "</td>\n";
        $retval .= "    <td>\n";
        ...
        $retval .= "</tr>\n";
        $retval .= "<tr>\n";
        $retval .= "    <td>" . __PMA_TRANSL('On completion preserve') . "</td>\n";
    ...
    public static function getQueryFromRequest()
    {
        ...
        if (! empty($_REQUEST['item_name'])) {
            ...
            $errors[] = __PMA_TRANSL('You must provide an event name!');
        }
        $query .= 'ON SCHEDULE ';
        if (! empty($_REQUEST['item_type'])
            ...
            if ($_REQUEST['item_type'] == 'RECURRING') {
                if (! empty($_REQUEST['item_interval_value'])
                    ...
                    $errors[]
                        = __PMA_TRANSL('You must provide a valid interval value for the event.');
                }
                ...
                if (! empty($_REQUEST['item_execute_at'])) {
                    ...
                    $errors[]
                        = __PMA_TRANSL('You must provide a valid execution time for the event.');
                }
            }
        } else {
            $errors[] = __PMA_TRANSL('You must provide a valid type for the event.');
        }
        $query .= 'ON COMPLETION ';
        ...
        if (! empty($_REQUEST['item_definition'])) {
            ...
            $errors[] = __PMA_TRANSL('You must provide an event definition.');
        }
