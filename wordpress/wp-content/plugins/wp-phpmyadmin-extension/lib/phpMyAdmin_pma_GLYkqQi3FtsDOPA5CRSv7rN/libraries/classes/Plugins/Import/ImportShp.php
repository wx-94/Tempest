...
class ImportShp extends ImportPlugin
{
    ...
    protected function setProperties()
    {
        ...
        $importPluginProperties->setText(__PMA_TRANSL('ESRI Shape File'));
        $importPluginProperties->setExtension('shp');
        $importPluginProperties->setOptions(array());
        $importPluginProperties->setOptionsText(__PMA_TRANSL('Options'));
    ...
    public function doImport(array &$sql_data = array())
    {
        ...
        if ($compression == 'application/zip'
            ...
            if ($GLOBALS['import_handle']->openZip('/^.*\.shp$/i') === false) {
                $message = Message::error(
                    __PMA_TRANSL('There was an error importing the ESRI shape file: "%s".')
                );
        ...
        default:
            $error = true;
            $message = Message::error(
                __PMA_TRANSL('MySQL Spatial Extension does not support ESRI type "%s".')
            );
        ...
        if (count($rows) == 0) {
            $error = true;
            $message = Message::error(
                __PMA_TRANSL('The imported file does not contain any data!')
            );
