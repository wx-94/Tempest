...
class Footer
{
    ...
    {
        $message = '<a href="/">' . __PMA_TRANSL('phpMyAdmin Demo Server') . '</a>: ';
        if (@file_exists('./revision-info.php')) {
            include './revision-info.php';
            $message .= sprintf(
                __PMA_TRANSL('Currently running Git revision %1$s from the %2$s branch.'),
            ...
            $message .= __PMA_TRANSL('Git information missing!');
        }
    ...
    private function _getSelfLink($url)
    {
        ...
        $retval .= '<a href="' . htmlspecialchars($url) . '"'
            . ' title="' . __PMA_TRANSL('Open new phpMyAdmin window') . '" target="_blank" rel="noopener noreferrer">';
        if (Util::showIcons('TabsMode')) {
            $retval .= Util::getImage(
                'window-new',
                __PMA_TRANSL('Open new phpMyAdmin window')
            );
        } else {
            $retval .=  __PMA_TRANSL('Open new phpMyAdmin window');
        }
        $retval .= '</a>';
        $retval .= '</div>';
        return $retval;
