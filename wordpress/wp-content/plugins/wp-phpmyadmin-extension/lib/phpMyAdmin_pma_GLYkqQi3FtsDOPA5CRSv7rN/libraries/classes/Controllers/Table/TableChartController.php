...
class TableChartController extends TableController
{
    ...
    public function indexAction()
    {
        ...
        if (!isset($this->sql_query) || $this->sql_query == '') {
            ...
            $this->response->addHTML(
                Message::error(__PMA_TRANSL('No SQL query was set to fetch data.'))
            );
            return;
        }
        ...
        if ($numeric_column_count == 0) {
            ...
            $this->response->addJSON(
                'message',
                __PMA_TRANSL('No numeric columns present in the table to plot.')
            );
            return;
        }
    ...
    public function ajaxAction()
    {
        ...
        if (empty($data)) {
            $this->response->setRequestStatus(false);
            $this->response->addJSON('message', __PMA_TRANSL('No data to display'));
            return;
        }
