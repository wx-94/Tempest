...
class Tracking
{
    ...
    public static function getHtmlForTableVersionDetails(
        ...
        $html .= '<tr>';
        $html .= '<th></th>';
        $html .= '<th>' . __PMA_TRANSL('Version') . '</th>';
        $html .= '<th>' . __PMA_TRANSL('Created') . '</th>';
        $html .= '<th>' . __PMA_TRANSL('Updated') . '</th>';
        $html .= '<th>' . __PMA_TRANSL('Status') . '</th>';
        $html .= '<th>' . __PMA_TRANSL('Action') . '</th>';
        $html .= '<th>' . __PMA_TRANSL('Show') . '</th>';
        $html .= '</tr>';
        $html .= '</thead>';
        $html .= '<tbody>';

        $GLOBALS['dbi']->dataSeek($sql_result, 0);
        $delete = Util::getIcon('b_drop', __PMA_TRANSL('Delete version'));
        $report = Util::getIcon('b_report', __PMA_TRANSL('Tracking report'));
        $structure = Util::getIcon('b_props', __PMA_TRANSL('Structure snapshot'));
        ...
        $html .= Util::getButtonOrImage(
            ...
            __PMA_TRANSL('Delete version'), 'b_drop', 'delete_version'
        );
    ...
    public static function getHtmlForTrackingReport($url_query, array $data, array $url_params,
        ...
        $html = '<h3>' . __PMA_TRANSL('Tracking report')
            . '  [<a href="tbl_tracking.php' . $url_query . '">' . __PMA_TRANSL('Close')
            . '</a>]</h3>';

        $html .= '<small>' . __PMA_TRANSL('Tracking statements') . ' '
        ...
        if (Util::showIcons('ActionLinksMode')) {
            $drop_image_or_text .= Util::getImage(
                'b_drop', __PMA_TRANSL('Delete tracking data row from report')
            );
        }
        if (Util::showText('ActionLinksMode')) {
            $drop_image_or_text .= __PMA_TRANSL('Delete');
        }
        ...
        if (count($data['ddlog']) == 0 && count($data['dmlog']) == 0) {
            $msg = Message::notice(__PMA_TRANSL('No data'));
            $msg->display();
        }
    ...
    public static function getHtmlForElementsOfTrackingReport(
        ...
        $str1 = '<select name="logtype">'
            ...
            . __PMA_TRANSL('Structure only') . '</option>'
            . '<option value="data"'
            . ($selection_data ? ' selected="selected"' : '') . '>'
            . __PMA_TRANSL('Data only') . '</option>'
            . '<option value="schema_and_data"'
            . ($selection_both ? ' selected="selected"' : '') . '>'
            . __PMA_TRANSL('Structure and data') . '</option>'
            . '</select>';
    ...
    public static function getHtmlForTrackingReportExportForm1(
        ...
        $html .= sprintf(
            __PMA_TRANSL('Show %1$s with dates from %2$s to %3$s by user %4$s %5$s'),
    ...
    public static function getHtmlForTrackingReportExportForm2(
        ...
        $str_export1 = '<select name="export_type">'
            . '<option value="sqldumpfile">' . __PMA_TRANSL('SQL dump (file download)')
            . '</option>'
            . '<option value="sqldump">' . __PMA_TRANSL('SQL dump') . '</option>'
            . '<option value="execution" onclick="alert(\''
            . Sanitize::escapeJsString(
                __PMA_TRANSL('This option will replace your table and contained data.')
            )
            . '\')">' . __PMA_TRANSL('SQL execution') . '</option>' . '</select>';

        $str_export2 = '<input type="hidden" name="report_export" value="1" />'
            . '<input type="submit" value="' . __PMA_TRANSL('Go') . '" />';

        $html .= "<br/>" . sprintf(__PMA_TRANSL('Export as %s'), $str_export1)
            . $str_export2 . "<br/>";
        $html .= '</form>';
        return $html;
    }
    ...
    public static function getHtmlForDataManipulationStatements(array $data, array $filter_users,
        ...
        list($html,) = self::getHtmlForDataStatements(
            $data, $filter_users, $filter_ts_from, $filter_ts_to, $url_params,
            $drop_image_or_text, 'dmlog', __PMA_TRANSL('Data manipulation statement'),
    ...
    public static function getHtmlForDataDefinitionStatements(array $data, array $filter_users,
        ...
        list($html, $line_number) = self::getHtmlForDataStatements(
            $data, $filter_users, $filter_ts_from, $filter_ts_to, $url_params,
            $drop_image_or_text, 'ddlog', __PMA_TRANSL('Data definition statement'),
            1, 'ddl_versions'
        );
    ...
    {
        $html = '<h3>' . __PMA_TRANSL('Structure snapshot')
            . '  [<a href="tbl_tracking.php' . $url_query . '">' . __PMA_TRANSL('Close')
            . '</a>]</h3>';
        ...
        $html .= Util::getMessage(
            sprintf(
                __PMA_TRANSL('Version %s snapshot (SQL code)'),
    ...
    public static function deleteTrackingReportRows(array &$data)
    {
        ...
        if (isset($_REQUEST['delete_ddlog'])) {
            ...
            $html .= self::deleteFromTrackingReportLog(
                ...
                'ddlog',
                'DDL',
                __PMA_TRANSL('Tracking data definition successfully deleted')
            );
        }
        ...
        if (isset($_REQUEST['delete_dmlog'])) {
            ...
            $html .= self::deleteFromTrackingReportLog(
                ...
                'dmlog',
                'DML',
                __PMA_TRANSL('Tracking data manipulation successfully deleted')
            );
        }
        return $html;
    }
    ...
    public static function deleteFromTrackingReportLog(array &$data, $which_log, $type, $message)
    {
        ...
        if ($delete_id == (int)$delete_id) {
            ...
            if ($successfullyDeleted) {
                ...
                $msg = Message::rawError(__PMA_TRANSL('Query error'));
            }
            $html .= $msg->getDisplay();
        }
        return $html;
    ...
    public static function exportAsSqlDump(array $entries)
    {
        ...
        $new_query = "# "
            ...
            . "\n"
            . "# " . __PMA_TRANSL('Comment out these two lines if you do not need them.') . "\n"
            . "\n"
        ...
        $msg = Message::success(
            __PMA_TRANSL('SQL statements exported. Please copy the dump or execute it.')
        );
        $html .= $msg->getDisplay();
    ...
    public static function exportAsFileDownload(array $entries)
    {
        ...
        $dump = "# " . sprintf(
            __PMA_TRANSL('Tracking report for table `%s`'), $table
        )
        . "\n" . "# " . date('Y-m-d H:i:s') . "\n";
        foreach ($entries as $entry) {
    ...
    public static function changeTracking($action)
    {
        ...
        if ($action == 'activate') {
            ...
            $message = __PMA_TRANSL('Tracking for %1$s was activated at version %2$s.');
        } else {
            $method = 'deactivateTracking';
            $message = __PMA_TRANSL('Tracking for %1$s was deactivated at version %2$s.');
        }
    ...
    public static function deleteTrackingVersion($version)
    {
        ...
        if ($versionDeleted) {
            $msg = Message::success(
                sprintf(
                    __PMA_TRANSL('Version %1$s of %2$s was deleted.'),
    ...
    public static function createTrackingVersion()
    {
        ...
        if ($versionCreated) {
            $msg = Message::success(
                sprintf(
                    __PMA_TRANSL('Version %1$s was created, tracking for %2$s is active.'),
                    htmlspecialchars($_REQUEST['version']),
                    htmlspecialchars($GLOBALS['db'] . '.' . $GLOBALS['table'])
                )
            );
    ...
    {
        if ($version['tracking_active'] == 1) {
            return __PMA_TRANSL('active');
        }

        return __PMA_TRANSL('not active');
    }
    ...
    private static function getStatusButton(array $versionData, $urlQuery)
    {
        ...
        $options = array(
            0 => array(
                'label' => __PMA_TRANSL('not active'),
                'value' => 'deactivate_now',
                'selected' => ($state != 'active')
            ),
            1 => array(
                'label' => __PMA_TRANSL('active'),
                'value' => 'activate_now',
                'selected' => ($state == 'active')
            )
        );
