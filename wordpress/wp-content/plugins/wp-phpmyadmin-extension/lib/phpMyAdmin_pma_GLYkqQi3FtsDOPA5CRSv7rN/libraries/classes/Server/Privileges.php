...
class Privileges
{
    ...
    public static function extractPrivInfo($row = null, $enableHTML = false, $tablePrivs = false)
    {
        ...
        if (empty($privs)) {
            if ($enableHTML) {
                $privs[] = '<dfn title="' . __PMA_TRANSL('No privileges.') . '">USAGE</dfn>';
            } else {
                $privs[] = 'USAGE';
            }
            ...
            if ($enableHTML) {
                $privs = array('<dfn title="'
                    . __PMA_TRANSL('Includes all privileges except GRANT.')
                    . '">ALL PRIVILEGES</dfn>'
                );
            } else {
    ...
    public static function getGrantsArray()
    {
        return array(
            array(
                'Select_priv',
                'SELECT',
                __PMA_TRANSL('Allows reading data.')
            ),
            array(
                'Insert_priv',
                'INSERT',
                __PMA_TRANSL('Allows inserting and replacing data.')
            ),
            array(
                'Update_priv',
                'UPDATE',
                __PMA_TRANSL('Allows changing data.')
            ),
            array(
                'Delete_priv',
                'DELETE',
                __PMA_TRANSL('Allows deleting data.')
            ),
            array(
                'Create_priv',
                'CREATE',
                __PMA_TRANSL('Allows creating new databases and tables.')
            ),
            array(
                'Drop_priv',
                'DROP',
                __PMA_TRANSL('Allows dropping databases and tables.')
            ),
            array(
                'Reload_priv',
                'RELOAD',
                __PMA_TRANSL('Allows reloading server settings and flushing the server\'s caches.')
            ),
            array(
                'Shutdown_priv',
                'SHUTDOWN',
                __PMA_TRANSL('Allows shutting down the server.')
            ),
            array(
                'Process_priv',
                'PROCESS',
                __PMA_TRANSL('Allows viewing processes of all users.')
            ),
            array(
                'File_priv',
                'FILE',
                __PMA_TRANSL('Allows importing data from and exporting data into files.')
            ),
            array(
                'References_priv',
                'REFERENCES',
                __PMA_TRANSL('Has no effect in this MySQL version.')
            ),
            array(
                'Index_priv',
                'INDEX',
                __PMA_TRANSL('Allows creating and dropping indexes.')
            ),
            array(
                'Alter_priv',
                'ALTER',
                __PMA_TRANSL('Allows altering the structure of existing tables.')
            ),
            array(
                'Show_db_priv',
                'SHOW DATABASES',
                __PMA_TRANSL('Gives access to the complete list of databases.')
            ),
            array(
                'Super_priv',
                'SUPER',
            ...
            array(
                ...
                __PMA_TRANSL('Allows creating temporary tables.')
            ),
            array(
                'Lock_tables_priv',
                'LOCK TABLES',
                __PMA_TRANSL('Allows locking tables for the current thread.')
            ),
            array(
                'Repl_slave_priv',
                'REPLICATION SLAVE',
                __PMA_TRANSL('Needed for the replication slaves.')
            ),
            array(
                'Repl_client_priv',
                'REPLICATION CLIENT',
                __PMA_TRANSL('Allows the user to ask where the slaves / masters are.')
            ),
            array(
                'Create_view_priv',
                'CREATE VIEW',
                __PMA_TRANSL('Allows creating new views.')
            ),
            array(
                'Event_priv',
                'EVENT',
                __PMA_TRANSL('Allows to set up events for the event scheduler.')
            ),
            array(
                'Trigger_priv',
                'TRIGGER',
                __PMA_TRANSL('Allows creating and dropping triggers.')
            ),
            // for table privs:
            array(
                'Create View_priv',
            ...
            array(
                'Show_view_priv',
                'SHOW VIEW',
                __PMA_TRANSL('Allows performing SHOW CREATE VIEW queries.')
            ),
            // for table privs:
            array(
                'Show view_priv',
            ...
            array(
                'Create_routine_priv',
                'CREATE ROUTINE',
                __PMA_TRANSL('Allows creating stored routines.')
            ),
            array(
                'Alter_routine_priv',
                'ALTER ROUTINE',
                __PMA_TRANSL('Allows altering and dropping stored routines.')
            ),
            array(
                'Create_user_priv',
                'CREATE USER',
                __PMA_TRANSL('Allows creating, dropping and renaming user accounts.')
            ),
            array(
                'Execute_priv',
                'EXECUTE',
                __PMA_TRANSL('Allows executing stored routines.')
            ),
        );
    }
    ...
    public static function getHtmlToDisplayPrivilegesTable($db = '*',
        ...
        if ($submit) {
            $html_output .= '<fieldset id="fieldset_user_privtable_footer" '
                ...
                . '<input type="submit" value="' . __PMA_TRANSL('Go') . '" />' . "\n"
                . '</fieldset>' . "\n";
        }
        return $html_output;
    ...
    public static function getHtmlForRoutineSpecificPrivileges(
        ...
        $privTableNames = array(__PMA_TRANSL('Routine'));
    ...
    public static function getTriggerPrivilegeTable()
    {
        $routinePrivTable = array(
            ...
            array(
                'Alter_routine',
                'ALTER ROUTINE',
                __PMA_TRANSL('Allows altering and dropping this routine.')
            ),
            array(
                'Execute',
                'EXECUTE',
                __PMA_TRANSL('Allows executing this routine.')
            )
        );
    ...
    public static function getHtmlForTableSpecificPrivileges(
        ...
        $html_output = '<input type="hidden" name="grant_count" '
            ...
            . '<legend data-submenu-label="' . __PMA_TRANSL('Table') . '">' . __PMA_TRANSL('Table-specific privileges')
            . '</legend>'
            . '<p><small><i>'
            . __PMA_TRANSL('Note: MySQL privilege names are expressed in English.')
            . '</i></small></p>';
    ...
    public static function getHtmlForAttachedPrivilegesToTableSpecificColumn(array $columns, array $row)
    {
        $html_output = self::getHtmlForColumnPrivileges(
            ...
            'select', __PMA_TRANSL('Allows reading data.'), 'Select'
        );

        $html_output .= self::getHtmlForColumnPrivileges(
            $columns, $row, 'Insert_priv', 'INSERT',
            'insert', __PMA_TRANSL('Allows inserting and replacing data.'), 'Insert'
        );

        $html_output .= self::getHtmlForColumnPrivileges(
            $columns, $row, 'Update_priv', 'UPDATE',
            'update', __PMA_TRANSL('Allows changing data.'), 'Update'
        );

        $html_output .= self::getHtmlForColumnPrivileges(
            $columns, $row, 'References_priv', 'REFERENCES', 'references',
            __PMA_TRANSL('Has no effect in this MySQL version.'), 'References'
        );
        return $html_output;
    }
    ...
    {
        $privTable_names = array(0 => __PMA_TRANSL('Data'),
            1 => __PMA_TRANSL('Structure'),
            2 => __PMA_TRANSL('Administration')
        );
        $privTable = array();
        // d a t a
        ...
        $html_output = '<input type="hidden" name="grant_count" value="'
            ...
            . '" />';
        if ($db == '*') {
            $legend     = __PMA_TRANSL('Global privileges');
            $menu_label = __PMA_TRANSL('Global');
        } elseif ($table == '*') {
            $legend     = __PMA_TRANSL('Database-specific privileges');
            $menu_label = __PMA_TRANSL('Database');
        } else {
            $legend     = __PMA_TRANSL('Table-specific privileges');
            $menu_label = __PMA_TRANSL('Table');
        }
        $html_output .= '<fieldset id="fieldset_user_global_rights">'
            ...
            . 'class="checkall_box" title="' . __PMA_TRANSL('Check all') . '" /> '
    ...
    public static function getDataPrivilegeTable($db)
    {
        $data_privTable = array(
            array('Select', 'SELECT', __PMA_TRANSL('Allows reading data.')),
            array('Insert', 'INSERT', __PMA_TRANSL('Allows inserting and replacing data.')),
            array('Update', 'UPDATE', __PMA_TRANSL('Allows changing data.')),
            array('Delete', 'DELETE', __PMA_TRANSL('Allows deleting data.'))
        );
        if ($db == '*') {
            $data_privTable[]
                = array('File',
    ...
    public static function getStructurePrivilegeTable($table, array $row)
    {
        $structure_privTable = array(
            array('Create',
                ...
                ($table == '*'
                    ? __PMA_TRANSL('Allows creating new databases and tables.')
                    : __PMA_TRANSL('Allows creating new tables.')
                )
            ),
            array('Alter',
                'ALTER',
            ...
            array('Index', 'INDEX', __PMA_TRANSL('Allows creating and dropping indexes.')),
            array('Drop',
                'DROP',
                ($table == '*'
                    ? __PMA_TRANSL('Allows dropping databases and tables.')
                    : __PMA_TRANSL('Allows dropping tables.')
                )
            ),
            ...
            array('Execute', 'EXECUTE', __PMA_TRANSL('Allows executing stored routines.')),
        );
    ...
    public static function getHtmlForAuthPluginsDropdown(
        ...
        if ($versions == 'new') {
            ...
            $active_auth_plugins = array(
                'mysql_native_password' => __PMA_TRANSL('Native MySQL authentication')
            );
        }
    ...
    public static function getActiveAuthPlugins()
    {
        ...
        while ($row = $GLOBALS['dbi']->fetchAssoc($resultset)) {
            ...
            if ('mysql_native_password' == $row['PLUGIN_NAME']) {
                $row['PLUGIN_DESCRIPTION'] = __PMA_TRANSL('Native MySQL authentication');
            } elseif ('sha256_password' == $row['PLUGIN_NAME']) {
                $row['PLUGIN_DESCRIPTION'] = __PMA_TRANSL('SHA256 password authentication');
            }
    ...
    public static function getHtmlForLoginInformationFields(
        ...
        $html_output .= '<input type="text" name="username" id="pma_username" class="autofocus"'
            . ' maxlength="' . $username_length . '" title="' . __PMA_TRANSL('User name') . '"'
        ...
        $html_output .= '<option value="userdefined"'
            . ((isset($GLOBALS['pred_hostname'])
                ...
                ? ' selected="selected"'
                : '') . '>'
            . __PMA_TRANSL('Use text field:') . '</option>' . "\n"
            . '</select>' . "\n"
            . '</span>' . "\n";
        ...
        $html_output .= '<div class="item">' . "\n"
            ...
            . '    ' . __PMA_TRANSL('Password:') . "\n"
            . '</label>' . "\n"
            . '<span class="options">' . "\n"
            . '<select name="pred_password" id="select_pred_password" title="'
            . __PMA_TRANSL('Password') . '">' . "\n"
            . ($mode == 'change' ? '<option value="keep" selected="selected">'
                . __PMA_TRANSL('Do not change the password')
                . '</option>' . "\n" : '')
            . '<option value="none"';
        ...
        $html_output .= '>' . __PMA_TRANSL('No Password') . '</option>' . "\n"
        ...
        if (($serverType == 'MySQL'
            ...
            $html_output .= __PMA_TRANSL('Authentication Plugin')
            ...
            $html_output .= __PMA_TRANSL('Password Hashing Method')
                . '</label><span class="options">&nbsp;</span>' . "\n";
            $auth_plugin_dropdown = self::getHtmlForAuthPluginsDropdown(
    ...
    public static function updatePassword($err_url, $username, $hostname)
    {
        ...
        if (empty($_REQUEST['nopass'])
            ...
            if ($_POST['pma_pw'] != $_POST['pma_pw2']) {
                $message = Message::error(__PMA_TRANSL('The passwords aren\'t the same!'));
            } elseif (empty($_POST['pma_pw']) || empty($_POST['pma_pw2'])) {
                $message = Message::error(__PMA_TRANSL('The password is empty!'));
            }
        }
        ...
        if (empty($message)) {
            ...
            $message = Message::success(
                __PMA_TRANSL('The password for %s was changed successfully.')
            );
    ...
    public static function getMessageAndSqlQueryForPrivilegesRevoke($dbname,
        ...
        $message = Message::success(
            __PMA_TRANSL('You have revoked the privileges for %s.')
        );
        $message->addParam('\'' . $username . '\'@\'' . $hostname . '\'');
    ...
    {
        $html_output = '<h2>' . "\n"
           . Util::getIcon('b_usradd') . __PMA_TRANSL('Add user account') . "\n"
           . '</h2>' . "\n"
        ...
        $html_output .= '<fieldset id="fieldset_add_user_database">' . "\n"
            . '<legend>' . __PMA_TRANSL('Database for user account') . '</legend>' . "\n";

        $html_output .= Template::get('checkbox')
            ->render(
                array(
                    'html_field_name'   => 'createdb-1',
                    'label'             => __PMA_TRANSL('Create database with same name and grant all privileges.'),
                    'checked'           => false,
                    'onclick'           => false,
        ...
        $html_output .= Template::get('checkbox')
            ->render(
                array(
                    ...
                    'label'             => __PMA_TRANSL('Grant all privileges on wildcard name (username\\_%).'),
                    'checked'           => false,
                    'onclick'           => false,
            ...
            $html_output .= Template::get('checkbox')
                ->render(
                    array(
                        ...
                        'label'             => sprintf(__PMA_TRANSL('Grant all privileges on database %s.'), htmlspecialchars($dbname)),
                        'checked'           => true,
                        'onclick'           => false,
        ...
        $html_output .= '<fieldset id="fieldset_add_user_footer" class="tblFooters">'
            ...
            . '<input type="submit" id="adduser_submit" value="' . __PMA_TRANSL('Go') . '" />'
            . "\n"
            . '</fieldset>' . "\n"
            . '</form>' . "\n";
    ...
    public static function getHtmlForSpecificDbPrivileges($db)
    {
        ...
        if ($GLOBALS['dbi']->isSuperuser()) {
            ...
            $html_output .= '<legend>' . "\n"
                ...
                . '    '
                . sprintf(
                    __PMA_TRANSL('Users having access to "%s"'),
            ...
            $html_output .= Util::getButtonOrImage(
                ...
                __PMA_TRANSL('Export'), 'b_tblexport', 'export'
            );
        ...
        if ($response->isAjax() == true
            ...
            $message = Message::success(__PMA_TRANSL('User has been added.'));
    ...
    public static function getHtmlForPrivsTableHead()
    {
        return '<thead>'
            . '<tr>'
            . '<th></th>'
            . '<th>' . __PMA_TRANSL('User name') . '</th>'
            . '<th>' . __PMA_TRANSL('Host name') . '</th>'
            . '<th>' . __PMA_TRANSL('Type') . '</th>'
            . '<th>' . __PMA_TRANSL('Privileges') . '</th>'
            . '<th>' . __PMA_TRANSL('Grant') . '</th>'
            . '<th colspan="2">' . __PMA_TRANSL('Action') . '</th>'
            . '</tr>'
            . '</thead>';
    }
    ...
    public static function getHtmlForViewUsersError()
    {
        return Message::error(
            __PMA_TRANSL('Not enough privilege to view users.')
        )->getDisplay();
    }
    ...
    public static function getHtmlListOfPrivs(
        ...
        for ($i = 0; $i < $nbPrivileges; $i++) {
            ...
            $html_output .= '<td>';
            if ($current['Type'] == 'g') {
                $html_output .= __PMA_TRANSL('global');
            } elseif ($current['Type'] == 'd') {
                if ($current['Db'] == Util::escapeMysqlWildcards($db)) {
                    $html_output .= __PMA_TRANSL('database-specific');
                } else {
                    $html_output .= __PMA_TRANSL('wildcard') . ': '
                        . '<code>'
                        . htmlspecialchars($current['Db'])
                        . '</code>';
                }
            } elseif ($current['Type'] == 't') {
                $html_output .= __PMA_TRANSL('table-specific');
            }
            $html_output .= '</td>';
            ...
            $html_output .= ($containsGrant ? __PMA_TRANSL('Yes') : __PMA_TRANSL('No'));
            $html_output .= '</td>';
    ...
    public static function getUserLink(
        ...
        switch($linktype) {
        case 'edit':
            $html .= Util::getIcon('b_usredit', __PMA_TRANSL('Edit privileges'));
            break;
        case 'revoke':
            $html .= Util::getIcon('b_usrdrop', __PMA_TRANSL('Revoke'));
            break;
        case 'export':
            $html .= Util::getIcon('b_tblexport', __PMA_TRANSL('Export'));
            break;
        }
        $html .= '</a>';
    ...
    public static function getUserGroupEditLink($username)
    {
         return '<a class="edit_user_group_anchor ajax"'
            ...
            . '">'
            . Util::getIcon('b_usrlist', __PMA_TRANSL('Edit user group'))
            . '</a>';
    }
        ...
        if (isset($_REQUEST['change_copy'])) {
            ...
            $new_user_string = '<tr>' . "\n"
                ...
                . (empty($_REQUEST['username'])
                        ? '<span style="color: #FF0000">' . __PMA_TRANSL('Any') . '</span>'
            ...
            if (! empty($password) || isset($_POST['pma_pw'])) {
                $new_user_string .= __PMA_TRANSL('Yes');
            } else {
            ...
            if ((isset($_POST['Grant_priv']) && $_POST['Grant_priv'] == 'Y')) {
                ...
                $new_user_string .= __PMA_TRANSL('No');
            }
            $new_user_string .='</td>';
    ...
    {
        $choices = array(
            '4' => __PMA_TRANSL('… keep the old one.'),
            '1' => __PMA_TRANSL('… delete the old one from the user tables.'),
            '2' => __PMA_TRANSL(
        ...
        $html_output .= '<fieldset id="fieldset_change_copy_user">' . "\n"
            . '<legend data-submenu-label="' . __PMA_TRANSL('Login Information') . '">' . "\n"
            . __PMA_TRANSL('Change login information / Copy user account')
            . '</legend>' . "\n"
            . self::getHtmlForLoginInformationFields('change', $username, $hostname);

        $html_output .= '<fieldset id="fieldset_mode">' . "\n"
            . ' <legend>'
            . __PMA_TRANSL('Create a new user account with the same privileges and …')
            . '</legend>' . "\n";
    ...
    {
        $html_output = '[ ' . __PMA_TRANSL('Database')
            . ' <a href="' . Util::getScriptNameForOption(
                $GLOBALS['cfg']['DefaultTabDatabase'], 'database'
            )
            . Url::getCommon(
        ...
        if (strlen($tablename) > 0) {
            $html_output .= ' [ ' . __PMA_TRANSL('Table') . ' <a href="'
    ...
    public static function getHtmlForAllTableSpecificRights(
        ...
        $uiData = array(
            'database' => array(
                ...
                'sub_menu_label' => __PMA_TRANSL('Database'),
                'legend'         => __PMA_TRANSL('Database-specific privileges'),
                'type_label'     => __PMA_TRANSL('Database'),
            ),
            'table' => array(
                'form_id'        => 'table_specific_priv',
                'sub_menu_label' => __PMA_TRANSL('Table'),
                'legend'         => __PMA_TRANSL('Table-specific privileges'),
                'type_label'     => __PMA_TRANSL('Table'),
            ),
            'routine' => array(
                'form_id'        => 'routine_specific_priv',
                'sub_menu_label' => __PMA_TRANSL('Routine'),
                'legend'         => __PMA_TRANSL('Routine-specific privileges'),
                'type_label'     => __PMA_TRANSL('Routine'),
            ),
        );
    ...
    public static function getUsersOverview($result, array $db_rights, $pmaThemeImage, $text_dir)
    {
        ...
        $html_output
            ...
            . '<th>' . __PMA_TRANSL('User name') . '</th>' . "\n"
            . '<th>' . __PMA_TRANSL('Host name') . '</th>' . "\n"
            . '<th>' . __PMA_TRANSL('Password') . '</th>' . "\n"
            . '<th>' . __PMA_TRANSL('Global privileges') . ' '
            . Util::showHint(
                __PMA_TRANSL('Note: MySQL privilege names are expressed in English.')
            )
            . '</th>' . "\n";
        if ($GLOBALS['cfgRelation']['menuswork']) {
            $html_output .= '<th>' . __PMA_TRANSL('User group') . '</th>' . "\n";
        }
        $html_output .= '<th>' . __PMA_TRANSL('Grant') . '</th>' . "\n"
            . '<th colspan="' . ($user_group_count > 0 ? '3' : '2') . '">'
            . __PMA_TRANSL('Action') . '</th>' . "\n"
            . '</tr>' . "\n"
            . '</thead>' . "\n";
    ...
    public static function getHtmlTableBodyForUserRights(array $db_rights)
    {
        ...
        foreach ($db_rights as $user) {
            ...
            foreach ($user as $host) {
                ...
                case 'Y':
                    $html_output .= __PMA_TRANSL('Yes');
                    break;
                case 'N':
                    $html_output .= '<span style="color: #FF0000">' . __PMA_TRANSL('No')
                        . '</span>';
                    break;
                ...
                if (! isset($host['Select_priv'])) {
                    $html_output .= Util::showHint(
                        __PMA_TRANSL('The selected user was not found in the privilege table.')
                    );
                }
                ...
                $html_output .= '<td>'
                    . ($host['Grant_priv'] == 'Y' ? __PMA_TRANSL('Yes') : __PMA_TRANSL('No'))
                    . '</td>' . "\n";
    ...
    public static function deleteUser(array $queries)
    {
        $sql_query = '';
        if (empty($queries)) {
            $message = Message::error(__PMA_TRANSL('No users selected for deleting!'));
        } else {
            if ($_REQUEST['mode'] == 3) {
                $queries[] = '# ' . __PMA_TRANSL('Reloading the privileges') . ' …';
            ...
            if (! empty($drop_user_error)) {
                ...
                $message = Message::success(
                    __PMA_TRANSL('The selected users have been deleted successfully.')
                );
            }
        }
    ...
    public static function updatePrivileges($username, $hostname, $tablename, $dbname, $itemType)
    {
        ...
        $message = Message::success(__PMA_TRANSL('You have updated the privileges for %s.'));
        $message->addParam('\'' . $username . '\'@\'' . $hostname . '\'');
    ...
    public static function getDataForChangeOrCopyUser()
    {
        ...
        if (isset($_REQUEST['change_copy'])) {
            ...
            if (! $row) {
                $response = Response::getInstance();
                $response->addHTML(
                    Message::notice(__PMA_TRANSL('No user found.'))->getDisplay()
                );
    ...
    public static function updateMessageForReload()
    {
        ...
        if (isset($_REQUEST['flush_privileges'])) {
            ...
            $message = Message::success(
                __PMA_TRANSL('The privileges were reloaded successfully.')
            );
        }
    ...
    public static function addUser(
        ...
        if ($GLOBALS['dbi']->fetchValue($sql) == 1) {
            $message = Message::error(__PMA_TRANSL('The user %s already exists!'));
    ...
    public static function getListForExportUserDefinition($username, $hostname)
    {
        ...
        if (isset($_REQUEST['selected_usr'])) {
            ...
            $title = __PMA_TRANSL('User') . ' `' . htmlspecialchars($username)
                . '`@`' . htmlspecialchars($hostname) . '`';
    ...
    public static function getHtmlHeaderForUserProperties(
        ...
        $html_output = '<h2>' . "\n"
           ...
           . __PMA_TRANSL('Edit privileges:') . ' '
           . __PMA_TRANSL('User account');
        ...
        if (! empty($dbname)) {
            ...
            $html_output .= ($dbname_is_wildcard
                ...
                ? __PMA_TRANSL('Databases') : __PMA_TRANSL('Database');
            if (! empty($entity_name) && $entity_type === 'table') {
                ...
                $html_output .= ' - ' . __PMA_TRANSL('Table')
                ...
                $html_output .= ' - ' . __PMA_TRANSL('Routine')
                    . ' <i>' . htmlspecialchars($entity_name) . '</i>';
            } else {
                if (! is_array($dbname)) {
                    $dbname = array($dbname);
    ...
    public static function getHtmlForUserOverview($pmaThemeImage, $text_dir)
    {
        $html_output = '<h2>' . "\n"
           ...
           . __PMA_TRANSL('User accounts overview') . "\n"
           . '</h2>' . "\n";
    ...
    public static function addUserAndCreateDatabase($_error, $real_sql_query, $sql_query,
        ...
        if ($_error || (!empty($real_sql_query)
            ...
            $message = Message::success(__PMA_TRANSL('You have added a new user.'));
        }
