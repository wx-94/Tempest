...
class MultSubmits
{
    ...
    public function getHtmlForCopyMultipleTables($action, array $urlParams)
    {
        ...
        $html .= '<strong><label for="db_name_dropdown">' . __PMA_TRANSL('Database') . ':</label></strong>';
        $html .= '<select id="db_name_dropdown" class="halfWidth" name="target_db" >'
            . $databasesList->getHtmlOptions(true, false)
            . '</select>';
        $html .= '<br><br>';
        $html .= '<strong><label>' . __PMA_TRANSL('Options') . ':</label></strong><br>';
        $html .= '<input type="radio" id ="what_structure" value="structure" name="what"/>';
        $html .= '<label for="what_structure">' . __PMA_TRANSL('Structure only') . '</label><br>';
        $html .= '<input type="radio" id ="what_data" value="data" name="what" checked="checked"/>';
        $html .= '<label for="what_data">' . __PMA_TRANSL('Structure and data') . '</label><br>';
        $html .= '<input type="radio" id ="what_dataonly" value="dataonly" name="what"/>';
        $html .= '<label for="what_dataonly">' . __PMA_TRANSL('Data only') . '</label><br><br>';
        $html .= '<input type="checkbox" id="checkbox_drop" value="1" name="drop_if_exists"/>';
        $html .= '<label for="checkbox_drop">' . __PMA_TRANSL('Add DROP TABLE') . '</label><br>';
        $html .= '<input type="checkbox" id="checkbox_auto_increment_cp" value="1" name="sql_auto_increment"/>';
        $html .= '<label for="checkbox_auto_increment_cp">' . __PMA_TRANSL('Add AUTO INCREMENT value') . '</label><br>';
        $html .= '<input type="checkbox" id="checkbox_constraints" value="1" name="sql_auto_increment" checked="checked"/>';
        $html .= '<label for="checkbox_constraints">' . __PMA_TRANSL('Add constraints') . '</label><br><br>';
        $html .= '<input name="adjust_privileges" value="1" id="checkbox_adjust_privileges" checked="checked" type="checkbox"/>';
        $html .= '<label for="checkbox_adjust_privileges">' . __PMA_TRANSL('Adjust privileges') . '<a href="./doc/html/faq.html#faq6-39" target="documentation"><img src="themes/dot.gif" title="Documentation" alt="Documentation" class="icon ic_b_help"></a></label>';
        $html .= '</fieldset>';
        $html .= '<input type="hidden" name="mult_btn" value="' . __PMA_TRANSL('Yes') . '" />';
        $html .= '</form>';
        return $html;
    }
    ...
    public function getHtmlForReplacePrefixTable($action, array $urlParams)
    {
        ...
        $html .= '<table>';
        $html .= '<tr>';
        $html .= '<td>' . __PMA_TRANSL('From') . '</td>';
        $html .= '<td>';
        $html .= '<input type="text" name="from_prefix" id="initialPrefix" />';
        $html .= '</td>';
        $html .= '</tr>';
        $html .= '<tr>';
        $html .= '<td>' . __PMA_TRANSL('To') . '</td>';
        $html .= '<td>';
    ...
    public function getHtmlForAddPrefixTable($action, array $urlParams)
    {
        ...
        $html .= '<table>';
        $html .= '<tr>';
        $html .= '<td>' . __PMA_TRANSL('Add prefix') . '</td>';
        $html .= '<td>';
    ...
    public function getHtmlForOtherActions($what, $action, array $urlParams, $fullQuery)
    {
        ...
        $html .= '<legend>';
        if ($what == 'drop_db') {
            $html .=  __PMA_TRANSL('You are about to DESTROY a complete database!') . ' ';
        }
        $html .= __PMA_TRANSL('Do you really want to execute the following query?');
        $html .= '</legend>';
