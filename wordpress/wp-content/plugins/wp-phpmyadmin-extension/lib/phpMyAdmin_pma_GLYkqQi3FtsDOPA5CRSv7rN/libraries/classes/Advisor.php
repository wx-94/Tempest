...
class Advisor
{
    ...
    public function storeError($description, $exception)
    {
        $this->runResult['errors'][] = $description
            . ' '
            . sprintf(
                __PMA_TRANSL('Error when evaluating: %s'),
        ...
        foreach ($this->parseResult['rules'] as $rule) {
            ...
            if (isset($rule['precondition'])) {
                try {
                    ...
                    $this->storeError(
                        sprintf(
                            __PMA_TRANSL('Failed evaluating precondition for rule \'%s\'.'),
                            $rule['name']
                        ),
                        $e
                    );
            ...
            if (! $precond) {
                ...
                try {
                    ...
                    $this->storeError(
                        sprintf(
                            __PMA_TRANSL('Failed calculating value for rule \'%s\'.'),
                            $rule['name']
                        ),
                        $e
                    );
                ...
                try {
                    ...
                    $this->storeError(
                        sprintf(
                            __PMA_TRANSL('Failed running test for rule \'%s\'.'),
                            $rule['name']
                        ),
                        $e
                    );
        ...
        case 'fired':
            ...
            if (count($jst) > 1) {
                try {
                    ...
                    $this->storeError(
                        sprintf(
                            __PMA_TRANSL('Failed formatting string for rule \'%s\'.'),
                            $rule['name']
                        ),
                        $e
                    );
    ...
    public static function parseRulesFile()
    {
        ...
        if ($file === false) {
            $errors[] = sprintf(
                __PMA_TRANSL('Error in reading file: The file \'%s\' does not exist or is not readable!'),
                $filename
            );
        ...
        for ($i = 0; $i < $numLines; $i++) {
            ...
            if (substr($line, 0, 4) == 'rule') {
                ...
                if (preg_match("/rule\s'(.*)'( \[(.*)\])?$/", $line, $match)) {
                    ...
                    $errors[] = sprintf(
                        __PMA_TRANSL('Invalid rule declaration on line %s.'),
                        $i + 1
                    );
                }
                continue;
                ...
                if ($ruleLine == -1) {
                    $errors[] = sprintf(
                        __PMA_TRANSL('Unexpected characters on line %s.'),
                        $i + 1
                    );
                }
            }
    ...
    {
        if ($num >= 1) { // per second
            $per = __PMA_TRANSL('per second');
        } elseif ($num * 60 >= 1) { // per minute
            $num = $num * 60;
            $per = __PMA_TRANSL('per minute');
        } elseif ($num * 60 * 60 >= 1 ) { // per hour
            $num = $num * 60 * 60;
            $per = __PMA_TRANSL('per hour');
        } else {
            $num = $num * 60 * 60 * 24;
            $per = __PMA_TRANSL('per day');
        }
