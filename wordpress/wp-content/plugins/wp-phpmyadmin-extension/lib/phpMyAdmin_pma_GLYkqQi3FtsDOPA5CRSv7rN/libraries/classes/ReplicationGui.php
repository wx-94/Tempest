...
class ReplicationGui
{
    ...
    public static function getHtmlForMasterReplication()
    {
        ...
        if (! isset($_REQUEST['repl_clear_scr'])) {
            $html .= '<fieldset>';
            $html .= '<legend>' . __PMA_TRANSL('Master replication') . '</legend>';
            $html .= __PMA_TRANSL('This server is configured as master in a replication process.');
            $html .= '<ul>';
            $html .= '  <li><a href="#master_status_href" id="master_status_href">';
            $html .= __PMA_TRANSL('Show master status') . '</a>';
            $html .= self::getHtmlForReplicationStatusTable('master', true, false);
            $html .= '  </li>';

            $html .= '  <li><a href="#master_slaves_href" id="master_slaves_href">';
            $html .= __PMA_TRANSL('Show connected slaves') . '</a>';
            ...
            $html .= __PMA_TRANSL('Add slave replication user') . '</a></li>';
        }
    ...
    {
        $html  = '<fieldset>';
        $html .= '<legend>' . __PMA_TRANSL('Master configuration') . '</legend>';
        $html .= __PMA_TRANSL(
        ...
        $html .= '<option value="all">' . __PMA_TRANSL('Replicate all databases; Ignore:');
        $html .= '</option>';
        $html .= '<option value="ign">' . __PMA_TRANSL('Ignore all databases; Replicate:');
        $html .= '</option>';
        $html .= '</select>';
        $html .= '<br /><br />';
        $html .= __PMA_TRANSL('Please select databases:') . '<br />';
        ...
        $html .= '  <input type="submit" value="' . __PMA_TRANSL('Go') . '" id="goButton" />';
        $html .= ' </form>';
        $html .= '</fieldset>';
    ...
    public static function getHtmlForSlaveConfiguration(
        ...
        $html  = '<fieldset>';
        $html .= '<legend>' . __PMA_TRANSL('Slave replication') . '</legend>';
        /**
        ...
        if ($server_slave_multi_replication) {
            $html .= __PMA_TRANSL('Master connection:');
            ...
            $html .= '<option value="">' . __PMA_TRANSL('Default') . '</option>';
        ...
        if ($server_slave_status) {
            ...
            if ($server_slave_replication[0]['Slave_SQL_Running'] == 'No') {
                $html .= Message::error(
                    __PMA_TRANSL('Slave SQL Thread not running!')
                )->getDisplay();
            }
            if ($server_slave_replication[0]['Slave_IO_Running'] == 'No') {
                $html .= Message::error(
                    __PMA_TRANSL('Slave IO Thread not running!')
                )->getDisplay();
            }
            ...
            $html .= (($server_slave_replication[0]['Slave_IO_Running'] == 'No' ||
                       $server_slave_replication[0]['Slave_SQL_Running'] == 'No')
                     ? __PMA_TRANSL('Full start')
                     : __PMA_TRANSL('Full stop')) . ' </a></li>';
            $html .= '   <li><a class="ajax" id="reset_slave"'
                . ' href="' . $slave_control_reset_link . '">';
            $html .= __PMA_TRANSL('Reset slave') . '</a></li>';
            if ($server_slave_replication[0]['Slave_SQL_Running'] == 'No') {
                $html .= '   <li><a href="' . $slave_control_sql_link . '">';
                $html .= __PMA_TRANSL('Start SQL Thread only') . '</a></li>';
            } else {
                $html .= '   <li><a href="' . $slave_control_sql_link . '">';
                $html .= __PMA_TRANSL('Stop SQL Thread only') . '</a></li>';
            }
            if ($server_slave_replication[0]['Slave_IO_Running'] == 'No') {
                $html .= '   <li><a href="' . $slave_control_io_link . '">';
                $html .= __PMA_TRANSL('Start IO Thread only') . '</a></li>';
            } else {
                $html .= '   <li><a href="' . $slave_control_io_link . '">';
                $html .= __PMA_TRANSL('Stop IO Thread only') . '</a></li>';
            }
            $html .= '  </ul>';
            $html .= ' </div>';
            $html .= ' </li>';
            ...
            $html .=  __PMA_TRANSL('Change or reconfigure master server') . '</a></li>';
            $html .= '</ul>';
            $html .= '</div>';
    ...
    public static function getHtmlForSlaveErrorManagement($slave_skip_error_link)
    {
        ...
        $html .= __PMA_TRANSL('Error management:') . '</a>';
        $html .= ' <div id="slave_errormanagement_gui" class="hide">';
        $html .= Message::error(
            __PMA_TRANSL('Skipping errors might lead into unsynchronized master and slave!')
        )->getDisplay();
        $html .= '  <ul>';
        $html .= '   <li><a href="' . $slave_skip_error_link . '">';
        $html .= __PMA_TRANSL('Skip current error') . '</a></li>';
        $html .= '   <li>';
        $html .= '    <form method="post" action="server_replication.php">';
        $html .= Url::getHiddenInputs('', '');
        $html .= sprintf(
            __PMA_TRANSL('Skip next %s errors.'),
    ...
    public static function getHtmlForReplicationDbMultibox()
    {
        ...
        $multi_values .= '<a href="#" id="db_select_href">' . __PMA_TRANSL('Select all') . '</a>';
        $multi_values .= '&nbsp;/&nbsp;';
        $multi_values .= '<a href="#" id="db_reset_href">' . __PMA_TRANSL('Unselect all') . '</a>';
    ...
    public static function getHtmlForReplicationChangeMaster($submitname)
    {
        ...
        $html .= '  <legend>' . __PMA_TRANSL('Slave configuration');
        $html .= ' - ' . __PMA_TRANSL('Change or reconfigure master server') . '</legend>';
        $html .= __PMA_TRANSL(
        ...
        $html .= self::getHtmlForAddUserInputDiv(
            array('text'=>__PMA_TRANSL('User name:'), 'for'=>"text_username"),
            array(
                'type'=>'text',
                'name'=>'username',
                'id'=>'text_username',
        ...
        $html .= self::getHtmlForAddUserInputDiv(
            array('text'=>__PMA_TRANSL('Password:'), 'for'=>"text_pma_pw"),
            array(
                'type'=>'password',
                'name'=>'pma_pw',
                'id'=>'text_pma_pw',
        ...
        $html .= self::getHtmlForAddUserInputDiv(
            array('text'=>__PMA_TRANSL('Host:'), 'for'=>"text_hostname"),
            array(
                'type'=>'text',
                'name'=>'hostname',
                'id'=>'text_hostname',
        ...
        $html .= self::getHtmlForAddUserInputDiv(
            array('text'=>__PMA_TRANSL('Port:'), 'for'=>"text_port"),
            array(
                'type'=>'number',
                'name'=>'text_port',
                'id'=>'text_port',
    ...
    public static function getHtmlForReplicationStatusTable($type, $hidden = false, $title = true)
    {
        ...
        if ($title) {
            if ($type == 'master') {
                ...
                $html .= __PMA_TRANSL('Master status') . '</h4>';
            } else {
                $html .= '<h4><a name="replication_' . $type . '"></a>';
                $html .= __PMA_TRANSL('Slave status') . '</h4>';
            }
        } else {
            $html .= '<br />';
        }
        ...
        $html .= '   <thead>';
        $html .= '    <tr>';
        $html .= '     <th>' . __PMA_TRANSL('Variable') . '</th>';
        $html .= '        <th>' . __PMA_TRANSL('Value') . '</th>';
        $html .= '    </tr>';
        $html .= '   </thead>';
        $html .= '   <tbody>';
    ...
    public static function getHtmlForReplicationSlavesTable($hidden = false)
    {
        ...
        $html .= '    <thead>';
        $html .= '      <tr>';
        $html .= '        <th>' . __PMA_TRANSL('Server ID') . '</th>';
        $html .= '        <th>' . __PMA_TRANSL('Host') . '</th>';
        $html .= '      </tr>';
        $html .= '    </thead>';
        $html .= '    <tbody>';
    ...
    public static function getHtmlForReplicationMasterAddSlaveUser()
    {
        ...
        $html .= Url::getHiddenInputs('', '');
        $html .= '<fieldset id="fieldset_add_user_login">'
            . '<legend>' . __PMA_TRANSL('Add slave replication user') . '</legend>'
        ...
        $html .= '        <option value="any"'
            ...
            ? ' selected="selected"' : '') . '>' . __PMA_TRANSL('Any host')
            . '</option>'
            . '        <option value="localhost"'
            . ((isset($GLOBALS['pred_hostname'])
                && $GLOBALS['pred_hostname'] == 'localhost')
            ? ' selected="selected"' : '') . '>' . __PMA_TRANSL('Local')
            . '</option>';
        ...
        if (!empty($thishost)) {
            $html .= '        <option value="thishost"'
                ...
                ? ' selected="selected"' : '') . '>' . __PMA_TRANSL('This Host')
                . '</option>';
        }
        unset($thishost);
    ...
    public static function getHtmlForAddUserLoginForm($username_length)
    {
        $html = '<input type="hidden" name="grant_count" value="25" />'
            ...
            . $username_length . '" title="' . __PMA_TRANSL('User name') . '"'
    ...
    public static function getHtmlForTableInfoForm($hostname_length)
    {
        $html = '        <option value="hosttable"'
            ...
            ? ' selected="selected"' : '') . '>' . __PMA_TRANSL('Use Host Table')
            . '</option>'
            . '        <option value="userdefined"'
            . ((isset($GLOBALS['pred_hostname'])
                && $GLOBALS['pred_hostname'] == 'userdefined')
        ...
        $html .= '>' . __PMA_TRANSL('No Password') . '</option>'
            . '        <option value="userdefined"'
            ...
            . '    ' . __PMA_TRANSL('Generate password:')
            . '</label>'
            . '<span class="options">'
            . '    <input type="button" class="button" '
            . 'id="button_generate_password" value="' . __PMA_TRANSL('Generate')
    ...
    public static function handleControlRequest()
    {
        if (isset($_REQUEST['sr_take_action'])) {
            ...
            if (isset($_REQUEST['slave_changemaster']) && ! $GLOBALS['cfg']['AllowArbitraryServer']) {
                ...
                $_SESSION['replication']['sr_action_info'] = __PMA_TRANSL('Connection to server is disabled, please enable $cfg[\'AllowArbitraryServer\'] in phpMyAdmin configuration.');
                ...
                case 'start':
                    $messageSuccess = __PMA_TRANSL('Replication started successfully.');
                    $messageError = __PMA_TRANSL('Error starting replication.');
                    break;
                case 'stop':
                    $messageSuccess = __PMA_TRANSL('Replication stopped successfully.');
                    $messageError = __PMA_TRANSL('Error stopping replication.');
                    break;
                case 'reset':
                    $messageSuccess = __PMA_TRANSL('Replication resetting successfully.');
                    $messageError = __PMA_TRANSL('Error resetting replication.');
                    break;
                default:
                    $messageSuccess = __PMA_TRANSL('Success.');
                    $messageError = __PMA_TRANSL('Error.');
                    break;
                }
    ...
    public static function handleRequestForSlaveChangeMaster()
    {
        ...
        $_SESSION['replication']['sr_action_status'] = 'error';
        $_SESSION['replication']['sr_action_info'] = __PMA_TRANSL('Unknown error');
        ...
        if (! $link_to_master) {
            ...
            $_SESSION['replication']['sr_action_info'] = sprintf(
                __PMA_TRANSL('Unable to connect to master %s.'),
            ...
            if (empty($position)) {
                ...
                if (! Replication::slaveChangeMaster(
                    ...
                    $_SESSION['replication']['sr_action_status'] = 'error';
                    $_SESSION['replication']['sr_action_info']
                        = __PMA_TRANSL('Unable to change master!');
                } else {
                    $_SESSION['replication']['sr_action_status'] = 'success';
                    $_SESSION['replication']['sr_action_info'] = sprintf(
                        __PMA_TRANSL('Master server changed successfully to %s.'),
                        htmlspecialchars($sr['hostname'])
                    );
                }
            }
