...
class CentralColumns
{
        ...
        if (! empty($insQuery)) {
            foreach ($insQuery as $query) {
                if (!$this->dbi->tryQuery($query, DatabaseInterface::CONNECT_CONTROL)) {
                    $message = Message::error(__PMA_TRANSL('Could not add columns!'));
                    $message->addMessage(
                        Message::rawError(
    ...
    public function deleteColumnsFromList(
        ...
        if (!$this->dbi->tryQuery($query, DatabaseInterface::CONNECT_CONTROL)) {
            $message = Message::error(__PMA_TRANSL('Could not remove columns!'));
    ...
    public function updateOneColumn(
        ...
        if ($orig_col_name == "") {
            ...
            $def['Null'] = $col_isNull?__PMA_TRANSL('YES'):__PMA_TRANSL('NO');
            $def['Extra'] = $col_extra;
    ...
    public function getTableHeader($class = '', $title = '', $actionCount = 0)
    {
        ...
        $tableheader .= '<tr>'
            ...
            . __PMA_TRANSL('Name') . '<div class="sorticon"></div></th>'
            . '<th class="' . $class . '" title="' . $title . '" data-column="type">'
            . __PMA_TRANSL('Type') . '<div class="sorticon"></div></th>'
            . '<th class="' . $class . '" title="' . $title . '" data-column="length">'
            . __PMA_TRANSL('Length/Values') . '<div class="sorticon"></div></th>'
            . '<th class="' . $class . '" title="' . $title . '" data-column="default">'
            . __PMA_TRANSL('Default') . '<div class="sorticon"></div></th>'
            . '<th class="' . $class . '" title="' . $title . '" data-column="collation"'
            . '>' . __PMA_TRANSL('Collation') . '<div class="sorticon"></div></th>'
            . '<th class="' . $class . '" title="' . $title
            . '" data-column="attribute">'
            . __PMA_TRANSL('Attribute') . '<div class="sorticon"></div></th>'
            . '<th class="' . $class . '" title="' . $title . '" data-column="isnull">'
            . __PMA_TRANSL('Null') . '<div class="sorticon"></div></th>'
            . '<th class="' . $class . '" title="' . $title . '" data-column="extra">'
            . __PMA_TRANSL('A_I') . '<div class="sorticon"></div></th>'
            . '</tr>';
        $tableheader .= '</thead>';
        return $tableheader;
    }
    ...
    private function getHtmlForTableDropdown($db)
    {
        ...
        $selectHtml = '<select name="table-select" id="table-select">'
            ...
            . __PMA_TRANSL('Select a table') . '</option>';
    ...
    public function getHtmlForTableRow(array $row, $row_num, $db)
    {
        $tableHtml = '<tr data-rownum="' . $row_num . '" id="f_' . $row_num . '">'
            ...
            . '<a href="#">' . Util::getIcon('b_edit', __PMA_TRANSL('Edit')) . '</a></td>'
            . '<td class="del_row" data-rownum = "' . $row_num . '">'
            . '<a hrf="#">' . Util::getIcon('b_drop', __PMA_TRANSL('Delete')) . '</a>'
        ...
        $tableHtml .=
            ...
            . '<span>' . ($row['col_isNull'] ? __PMA_TRANSL('Yes') : __PMA_TRANSL('No'))
            . '</span>'
    ...
    public function getTableFooter($pmaThemeImage, $text_dir)
    {
        ...
        $html_output .= Util::getButtonOrImage(
            ...
            __PMA_TRANSL('Edit'), 'b_edit', 'edit central columns'
        );
        $html_output .= Util::getButtonOrImage(
            'delete_central_columns', 'mult_submit',
            __PMA_TRANSL('Delete'), 'b_drop',
    ...
    private function getEditTableFooter()
    {
        $html_output = '<fieldset class="tblFooters">'
            . '<input type="submit" '
            . 'name="save_multi_central_column_edit" value="' . __PMA_TRANSL('Save') . '" />'
            . '</fieldset>';
        return $html_output;
    }
    ...
    public function getHtmlForAddNewColumn($db, $total_rows)
    {
        $addNewColumn = '<div id="add_col_div" class="topmargin"><a href="#">'
            . '<span>+</span> ' . __PMA_TRANSL('Add new column') . '</a>'
    ...
    public function getHtmlForEditingPage(array $selected_fld, $selected_db)
    {
        ...
        $header_cells = array(
            __PMA_TRANSL('Name'), __PMA_TRANSL('Type'), __PMA_TRANSL('Length/Values'), __PMA_TRANSL('Default'),
            __PMA_TRANSL('Collation'), __PMA_TRANSL('Attributes'), __PMA_TRANSL('Null'), __PMA_TRANSL('A_I')
        );
