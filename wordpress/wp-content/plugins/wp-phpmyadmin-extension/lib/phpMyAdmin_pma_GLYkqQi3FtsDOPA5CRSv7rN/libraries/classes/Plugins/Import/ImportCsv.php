    ...
    protected function setProperties()
    {
        ...
        if ($GLOBALS['plugin_param'] !== 'table') {
            ...
            $leaf = new TextPropertyItem(
                "columns",
                __PMA_TRANSL('Column names: ') . Util::showHint($hint)
            );
            $generalOptions->addProperty($leaf);
        }
        ...
        $leaf = new BoolPropertyItem(
            "ignore",
            __PMA_TRANSL('Do not abort on INSERT error')
        );
        $generalOptions->addProperty($leaf);
    }
    ...
    public function doImport(array &$sql_data = array())
    {
        ...
        if (strlen($csv_terminated) === 0) {
            $message = Message::error(
                __PMA_TRANSL('Invalid parameter for CSV import: %s')
            );
            $message->addParam(__PMA_TRANSL('Columns terminated with'));
            $error = true;
            $param_error = true;
            ...
            $message->addParam(__PMA_TRANSL('Columns enclosed with'));
            $error = true;
            $param_error = true;
            ...
            $message->addParam(__PMA_TRANSL('Columns escaped with'));
            $error = true;
            $param_error = true;
            ...
            $message->addParam(__PMA_TRANSL('Lines terminated with'));
            $error = true;
            $param_error = true;
        }
        ...
        while (!($finished && $i >= $len) && !$error && !$timeout_passed) {
            ...
            while ($i < $len) {
                ...
                if ($lasti == $i && $lastlen == $len) {
                    $message = Message::error(
                        __PMA_TRANSL('Invalid format of CSV input on line %d.')
                    );
