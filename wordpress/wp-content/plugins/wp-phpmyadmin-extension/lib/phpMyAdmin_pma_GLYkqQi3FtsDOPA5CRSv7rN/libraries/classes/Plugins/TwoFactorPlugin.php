...
class TwoFactorPlugin
{
        ...
        if ($this->_provided) {
            if (!empty($this->_message)) {
                return Message::rawError(
                    sprintf(__PMA_TRANSL('Two-factor authentication failed: %s'), $this->_message)
                )->getDisplay();
            }
            return Message::rawError(
                __PMA_TRANSL('Two-factor authentication failed.')
            )->getDisplay();
        }
        return '';
    }
    ...
    {
        return __PMA_TRANSL('No Two-Factor');
    }
    ...
    {
        return __PMA_TRANSL('Login using password only.');
    }
