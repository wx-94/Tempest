...
class Config
{
        ...
        if (! @is_readable($this->getSource())) {
            ...
            if ($contents === false) {
                ...
                Core::fatalError(
                    sprintf(
                        function_exists('__')
                        ? __PMA_TRANSL('Existing configuration file (%s) is not readable.')
    ...
    public function checkErrors()
    {
        if ($this->error_config_default_file) {
            Core::fatalError(
                sprintf(
                    __PMA_TRANSL('Could not load default configuration from: %1$s'),
                    $this->default_source
                )
            );
        }
        ...
        if ($this->error_config_file) {
            $error = '[strong]' . __PMA_TRANSL('Failed to read configuration file!') . '[/strong]'
                . '[br][br]'
                . __PMA_TRANSL(
    ...
    protected static function getFontsizeSelection()
    {
        ...
        $return = '<label for="select_fontsize">' . __PMA_TRANSL('Font size')
            . ':</label>' . "\n"
        ...
        if (! isset($this->settings['Servers']) || count($this->settings['Servers']) == 0) {
            ...
            foreach ($this->settings['Servers'] as $server_index => $each_server) {
                ...
                if (!is_int($server_index) || $server_index < 1) {
                    trigger_error(
                        sprintf(__PMA_TRANSL('Invalid server index: %s'), $server_index),
                        E_USER_ERROR
                    );
                }
                ...
                if (empty($each_server['host']) && empty($each_server['verbose'])) {
                    $each_server['verbose'] = sprintf(__PMA_TRANSL('Server %d'), $server_index);
                }
