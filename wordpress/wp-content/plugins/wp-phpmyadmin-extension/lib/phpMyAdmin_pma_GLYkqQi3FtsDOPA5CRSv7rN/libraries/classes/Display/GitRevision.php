...
class GitRevision
{
    ...
    */
    public static function display()
    {
        ...
        if ($branch !== false) {
            $branch = sprintf(__PMA_TRANSL('%1$s from %2$s branch'), $commit_hash, $branch);
        } else {
            $branch = $commit_hash . ' (' . __PMA_TRANSL('no branch') . ')';
        }

        $committer = $GLOBALS['PMA_Config']->get('PMA_VERSION_GIT_COMMITTER');
        $author = $GLOBALS['PMA_Config']->get('PMA_VERSION_GIT_AUTHOR');
        Core::printListItem(
            __PMA_TRANSL('Git revision:') . ' '
            . $branch . ',<br /> '
            . sprintf(
                __PMA_TRANSL('committed on %1$s by %2$s'),
            ...
            . ($author != $committer
                ? ', <br />'
                . sprintf(
                    __PMA_TRANSL('authored on %1$s by %2$s'),
