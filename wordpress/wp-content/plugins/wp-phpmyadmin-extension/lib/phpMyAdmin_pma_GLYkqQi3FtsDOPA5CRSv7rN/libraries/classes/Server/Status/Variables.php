...
class Variables
{
    ...
    public static function getHtmlForFilter(Data $serverStatusData)
    {
        ...
        $retval .= '<legend>' . __PMA_TRANSL('Filters') . '</legend>';
        $retval .= '<form action="server_status_variables.php'
            . Url::getCommon() . '">';
        $retval .= '<input type="submit" value="' . __PMA_TRANSL('Refresh') . '" />';
        $retval .= '<div class="formelement">';
        $retval .= '<label for="filterText">' . __PMA_TRANSL('Containing the word:') . '</label>';
        ...
        $retval .= __PMA_TRANSL('Show only alert values');
        $retval .= '</label>';
        $retval .= '</div>';
        $retval .= '<div class="formelement">';
        $retval .= '<select id="filterCategory" name="filterCategory">';
        $retval .= '<option value="">' . __PMA_TRANSL('Filter by category…') . '</option>';
        ...
        $retval .= __PMA_TRANSL('Show unformatted values');
        $retval .= '</label>';
        $retval .= '</div>';
        $retval .= '</form>';
        $retval .= '</fieldset>';
    ...
    public static function getHtmlForLinkSuggestions(Data $serverStatusData)
    {
        ...
        $retval .= '<p class="notice">' . __PMA_TRANSL('Related links:');
    ...
    public static function getHtmlForRenderVariables(Data $serverStatusData, array $alerts, array $strShowStatus)
    {
        ...
        $retval .= '<thead>';
        $retval .= '<tr>';
        $retval .= '<th>' . __PMA_TRANSL('Variable') . '</th>';
        $retval .= '<th>' . __PMA_TRANSL('Value') . '</th>';
        $retval .= '<th>' . __PMA_TRANSL('Description') . '</th>';
        $retval .= '</tr>';
        $retval .= '</thead>';
        $retval .= '<tbody>';
    ...
    public static function getDescriptions()
    {
        ...
        return array(
            ...
            'Binlog_cache_use' => __PMA_TRANSL(
                'The number of transactions that used the temporary binary log cache.'
            ),
            'Connections' => __PMA_TRANSL(
                'The number of connection attempts (successful or not)'
                . ' to the MySQL server.'
            ),
            'Created_tmp_disk_tables' => __PMA_TRANSL(
            ...
            'Handler_read_first' => __PMA_TRANSL(
            ...
            'Handler_read_rnd_next' => __PMA_TRANSL(
            ...
            'Handler_rollback' => __PMA_TRANSL(
                'The number of internal ROLLBACK statements.'
            ),
            'Handler_update' => __PMA_TRANSL(
                'The number of requests to update a row in a table.'
            ),
            'Handler_write' => __PMA_TRANSL(
                'The number of requests to insert a row in a table.'
            ),
            'Innodb_buffer_pool_pages_data' => __PMA_TRANSL(
                'The number of pages containing data (dirty or clean).'
            ),
            'Innodb_buffer_pool_pages_dirty' => __PMA_TRANSL(
                'The number of pages currently dirty.'
            ),
            'Innodb_buffer_pool_pages_flushed' => __PMA_TRANSL(
                'The number of buffer pool pages that have been requested'
                . ' to be flushed.'
            ),
            'Innodb_buffer_pool_pages_free' => __PMA_TRANSL(
                'The number of free pages.'
            ),
            'Innodb_buffer_pool_pages_latched' => __PMA_TRANSL(
            ...
            'Innodb_buffer_pool_pages_misc' => __PMA_TRANSL(
            ...
            'Innodb_buffer_pool_write_requests' => __PMA_TRANSL(
                'The number writes done to the InnoDB buffer pool.'
            ),
            'Innodb_data_fsyncs' => __PMA_TRANSL(
                'The number of fsync() operations so far.'
            ),
            'Innodb_data_pending_fsyncs' => __PMA_TRANSL(
                'The current number of pending fsync() operations.'
            ),
            'Innodb_data_pending_reads' => __PMA_TRANSL(
                'The current number of pending reads.'
            ),
            'Innodb_data_pending_writes' => __PMA_TRANSL(
                'The current number of pending writes.'
            ),
            'Innodb_data_read' => __PMA_TRANSL(
                'The amount of data read so far, in bytes.'
            ),
            'Innodb_data_reads' => __PMA_TRANSL(
                'The total number of data reads.'
            ),
            'Innodb_data_writes' => __PMA_TRANSL(
                'The total number of data writes.'
            ),
            'Innodb_data_written' => __PMA_TRANSL(
                'The amount of data written so far, in bytes.'
            ),
            'Innodb_dblwr_pages_written' => __PMA_TRANSL(
                'The number of pages that have been written for'
                . ' doublewrite operations.'
            ),
            'Innodb_dblwr_writes' => __PMA_TRANSL(
                'The number of doublewrite operations that have been performed.'
            ),
            'Innodb_log_waits' => __PMA_TRANSL(
                'The number of waits we had because log buffer was too small and'
                . ' we had to wait for it to be flushed before continuing.'
            ),
            'Innodb_log_write_requests' => __PMA_TRANSL(
                'The number of log write requests.'
            ),
            'Innodb_log_writes' => __PMA_TRANSL(
                'The number of physical writes to the log file.'
            ),
            'Innodb_os_log_fsyncs' => __PMA_TRANSL(
                'The number of fsync() writes done to the log file.'
            ),
            'Innodb_os_log_pending_fsyncs' => __PMA_TRANSL(
                'The number of pending log file fsyncs.'
            ),
            'Innodb_os_log_pending_writes' => __PMA_TRANSL(
                'Pending log file writes.'
            ),
            'Innodb_os_log_written' => __PMA_TRANSL(
                'The number of bytes written to the log file.'
            ),
            'Innodb_pages_created' => __PMA_TRANSL(
                'The number of pages created.'
            ),
            'Innodb_page_size' => __PMA_TRANSL(
                'The compiled-in InnoDB page size (default 16KB). Many values are'
                . ' counted in pages; the page size allows them to be easily'
                . ' converted to bytes.'
            ),
            'Innodb_pages_read' => __PMA_TRANSL(
                'The number of pages read.'
            ),
            'Innodb_pages_written' => __PMA_TRANSL(
                'The number of pages written.'
            ),
            'Innodb_row_lock_current_waits' => __PMA_TRANSL(
                'The number of row locks currently being waited for.'
            ),
            'Innodb_row_lock_time_avg' => __PMA_TRANSL(
                'The average time to acquire a row lock, in milliseconds.'
            ),
            'Innodb_row_lock_time' => __PMA_TRANSL(
                'The total time spent in acquiring row locks, in milliseconds.'
            ),
            'Innodb_row_lock_time_max' => __PMA_TRANSL(
                'The maximum time to acquire a row lock, in milliseconds.'
            ),
            'Innodb_row_lock_waits' => __PMA_TRANSL(
                'The number of times a row lock had to be waited for.'
            ),
            'Innodb_rows_deleted' => __PMA_TRANSL(
                'The number of rows deleted from InnoDB tables.'
            ),
            'Innodb_rows_inserted' => __PMA_TRANSL(
                'The number of rows inserted in InnoDB tables.'
            ),
            'Innodb_rows_read' => __PMA_TRANSL(
                'The number of rows read from InnoDB tables.'
            ),
            'Innodb_rows_updated' => __PMA_TRANSL(
                'The number of rows updated in InnoDB tables.'
            ),
            'Key_blocks_not_flushed' => __PMA_TRANSL(
            ...
            'Key_buffer_fraction_%' => __PMA_TRANSL(
                'Percentage of used key cache (calculated value)'
            ),
            'Key_read_requests' => __PMA_TRANSL(
                'The number of requests to read a key block from the cache.'
            ),
            'Key_reads' => __PMA_TRANSL(
            ...
            'Qcache_free_memory' => __PMA_TRANSL(
                'The amount of free memory for query cache.'
            ),
            'Qcache_hits' => __PMA_TRANSL(
                'The number of cache hits.'
            ),
            'Qcache_inserts' => __PMA_TRANSL(
                'The number of queries added to the cache.'
            ),
            'Qcache_lowmem_prunes' => __PMA_TRANSL(
            ...
            'Sort_range' => __PMA_TRANSL(
                'The number of sorts that were done with ranges.'
            ),
            'Sort_rows' => __PMA_TRANSL(
                'The number of sorted rows.'
            ),
            'Sort_scan' => __PMA_TRANSL(
                'The number of sorts that were done by scanning the table.'
            ),
            'Table_locks_immediate' => __PMA_TRANSL(
                'The number of times that a table lock was acquired immediately.'
            ),
            'Table_locks_waited' => __PMA_TRANSL(
            ...
            'Threads_connected' => __PMA_TRANSL(
                'The number of currently open connections.'
            ),
            'Threads_created' => __PMA_TRANSL(
            ...
            'Threads_cache_hitrate_%' => __PMA_TRANSL(
                'Thread cache hit rate (calculated value)'
            ),
            'Threads_running' => __PMA_TRANSL(
