...
class Triggers
{
    ...
    public static function handleEditor()
    {
            ...
            if (! count($errors)) { // set by PhpMyAdmin\Rte\Routines::getQueryFromRequest()
                ...
                if (! empty($_REQUEST['editor_process_edit'])) {
                    ...
                    if (! $result) {
                        ...
                        if (! $result) {
                            ...
                            $message = Message::success(
                                __PMA_TRANSL('Trigger %1$s has been modified.')
                            );
                            $message->addParam(
                    ...
                    if (! $result) {
                        ...
                        $message = Message::success(
                            __PMA_TRANSL('Trigger %1$s has been created.')
                        );
                        $message->addParam(
                            Util::backquote($_REQUEST['item_name'])
                        );
        ...
        if (count($errors)
            ...
            if (! empty($_REQUEST['add_item'])) {
                ...
                $title = __PMA_TRANSL("Edit trigger");
    ...
    public static function getEditorForm($mode, array $item)
    {
        ...
        $retval .= "<tr>\n";
        $retval .= "    <td style='width: 20%;'>" . __PMA_TRANSL('Trigger name') . "</td>\n";
        $retval .= "    <td><input type='text' name='item_name' maxlength='64'\n";
        $retval .= "               value='{$item['item_name']}' /></td>\n";
        $retval .= "</tr>\n";
        $retval .= "<tr>\n";
        $retval .= "    <td>" . __PMA_TRANSL('Table') . "</td>\n";
        $retval .= "    <td>\n";
        ...
        $retval .= "</tr>\n";
        $retval .= "<tr>\n";
        $retval .= "    <td>" . __PMA_TRANSL('Event') . "</td>\n";
    ...
    public static function getQueryFromRequest()
    {
        ...
        if (! empty($_REQUEST['item_name'])) {
            ...
            $errors[] = __PMA_TRANSL('You must provide a trigger name!');
        }
        if (! empty($_REQUEST['item_timing'])
            ...
            $errors[] = __PMA_TRANSL('You must provide a valid timing for the trigger!');
        }
        if (! empty($_REQUEST['item_event'])
            ...
            $errors[] = __PMA_TRANSL('You must provide a valid event for the trigger!');
        }
        $query .= 'ON ';
        if (! empty($_REQUEST['item_table'])
            ...
            $errors[] = __PMA_TRANSL('You must provide a valid table name!');
        }
        $query .= ' FOR EACH ROW ';
        if (! empty($_REQUEST['item_definition'])) {
            $query .= $_REQUEST['item_definition'];
        } else {
            $errors[] = __PMA_TRANSL('You must provide a trigger definition.');
        }
