...
if ($cfgRelation['commwork']) {
     ...
     */
    if ($comment) {
        echo '<p>' , __PMA_TRANSL('Database comment')
...
foreach ($tables as $table) {
    ...
    if (!empty($show_comment)) {
        echo __PMA_TRANSL('Table comments:') , ' ';
    ...
    echo '<tr><th width="50">' , __PMA_TRANSL('Column') , '</th>';
    echo '<th width="80">' , __PMA_TRANSL('Type') , '</th>';
    echo '<th width="40">' , __PMA_TRANSL('Null') , '</th>';
    echo '<th width="70">' , __PMA_TRANSL('Default') , '</th>';
    if ($have_rel) {
        echo '    <th>' , __PMA_TRANSL('Links to') , '</th>' , "\n";
    }
    echo '    <th>' , __PMA_TRANSL('Comments') , '</th>' , "\n";
    ...
    foreach ($columns as $row) {
        ...
        if (isset($pk_array[$row['Field']])) {
            echo ' <em>(' , __PMA_TRANSL('Primary') , ')</em>';
        }
        echo '</td>';
        echo '<td'
        ...
        echo '<td>';
        echo (($row['Null'] == 'NO') ? __PMA_TRANSL('No') : __PMA_TRANSL('Yes'));
        echo '</td>';
        echo '<td class="nowrap">';
        if (isset($row['Default'])) {
            echo $row['Default'];
