...
if (isset($_REQUEST['console_bookmark_add'])) {
    if (isset($_REQUEST['label']) && isset($_REQUEST['db'])
        ...
        if ($bookmark !== false && $bookmark->save()) {
            $response->addJSON('message', __PMA_TRANSL('Succeeded'));
            $response->addJSON('data', $bookmarkFields);
            $response->addJSON('isShared', $isShared);
        } else {
            $response->addJSON('message', __PMA_TRANSL('Failed'));
        }
        die();
    } else {
        $response->addJSON('message', __PMA_TRANSL('Incomplete params'));
        die();
    }
}
...
if (! empty($_REQUEST['id_bookmark'])) {
    ...
    case 1: // bookmarked query that have to be displayed
        ...
        $import_text = $bookmark->getQuery();
        if ($response->isAjax()) {
            $message = PhpMyAdmin\Message::success(__PMA_TRANSL('Showing bookmark'));
        ...
        if (! empty($bookmark)) {
            ...
            if ($response->isAjax()) {
                $message = PhpMyAdmin\Message::success(
                    __PMA_TRANSL('The bookmark has been deleted.')
                );
                $response->setRequestStatus($message->isSuccess());
                $response->addJSON('message', $message);
                $response->addJSON('action_bookmark', $_REQUEST['action_bookmark']);
...
if (! $error) {
    ...
    if ($import_plugin == null) {
        $message = PhpMyAdmin\Message::error(
            __PMA_TRANSL('Could not load import plugins, please check your installation!')
        );
        Import::stop($message);
    } else {
        // Do the real import
...
if (! empty($id_bookmark) && $_REQUEST['action_bookmark'] == 2) {
    $message = PhpMyAdmin\Message::success(__PMA_TRANSL('The bookmark has been deleted.'));
    $display_query = $import_text;
    $error = false; // unset error marker, it was used just to skip processing
} elseif (! empty($id_bookmark) && $_REQUEST['action_bookmark'] == 1) {
    $message = PhpMyAdmin\Message::notice(__PMA_TRANSL('Showing bookmark'));
...
if ($go_sql) {
    ...
    foreach ($sql_queries as $sql_query) {
        ...
        if ($sql->hasNoRightsToDropDatabase(
            ...
            PhpMyAdmin\Util::mysqlDie(
                __PMA_TRANSL('"DROP DATABASE" statements are disabled.'),
                '',
                false,
