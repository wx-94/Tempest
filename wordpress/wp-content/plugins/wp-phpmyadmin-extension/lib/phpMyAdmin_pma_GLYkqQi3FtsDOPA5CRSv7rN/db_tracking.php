...
if (isset($_REQUEST['delete_tracking']) && isset($_REQUEST['table'])) {
    ...
    Message::success(
        __PMA_TRANSL('Tracking data deleted successfully.')
    )->display();
    ...
    if (! empty($_REQUEST['selected_tbl'])) {
        ...
        Message::notice(
            __PMA_TRANSL('No tables selected.')
        )->display();
    }
}
...
if ($num_tables == 0 && count($data['ddlog']) == 0) {
    echo '<p>' , __PMA_TRANSL('No tables found in database.') , '</p>' , "\n";
...
if (count($data['ddlog']) > 0) {
    ...
    echo Util::getMessage(__PMA_TRANSL('Database Log'), $log);
}
