 ...
 * Displays the form
 */
$export_page_title = __PMA_TRANSL('View dump (schema) of database');

// exit if no tables in db found
if ($num_tables < 1) {
    $response->addHTML(
        Message::error(__PMA_TRANSL('No tables found in database.'))->getDisplay()
    );
    exit;
} // end if
...
$multi_values .= '<table class="export_table_select">'
    . '<thead><tr><th></th>'
    . '<th>' . __PMA_TRANSL('Tables') . '</th>'
    . '<th class="export_structure">' . __PMA_TRANSL('Structure') . '</th>'
    . '<th class="export_data">' . __PMA_TRANSL('Data') . '</th>'
    . '</tr><tr>'
    . '<td></td>'
    . '<td class="export_table_name all">' . __PMA_TRANSL('Select all') . '</td>'
