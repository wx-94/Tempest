...
if (Tracker::isActive()
    ...
    $msg = Message::notice(
        sprintf(
            __PMA_TRANSL('Tracking of %s is activated.'),
    ...
    if (! empty($_REQUEST['selected_versions'])) {
        if ($_REQUEST['submit_mult'] == 'delete_version') {
            ...
            $html .= Message::success(
                __PMA_TRANSL('Tracking versions deleted successfully.')
            )->getDisplay();
        }
    } else {
        $html .= Message::notice(
            __PMA_TRANSL('No versions selected.')
        )->getDisplay();
    }
}
...
if (isset($_REQUEST['report_export']) && $_REQUEST['export_type'] == 'execution') {
    ...
    $msg = Message::success(__PMA_TRANSL('SQL statements executed.'));
    $html .= $msg->getDisplay();
}
