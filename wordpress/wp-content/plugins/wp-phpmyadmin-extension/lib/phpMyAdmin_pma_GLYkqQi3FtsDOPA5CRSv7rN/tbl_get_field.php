...
if (!$GLOBALS['dbi']->selectDb($db)) {
    PhpMyAdmin\Util::mysqlDie(
        sprintf(__PMA_TRANSL('\'%s\' database does not exist.'), htmlspecialchars($db)),
        '', false
    );
}
...
if (!$GLOBALS['dbi']->getColumns($db, $table)) {
    PhpMyAdmin\Util::mysqlDie(__PMA_TRANSL('Invalid table name'));
}
...
if ($result === false) {
    PhpMyAdmin\Util::mysqlDie(
        __PMA_TRANSL('MySQL returned an empty result set (i.e. zero rows).'), $sql
    );
}
