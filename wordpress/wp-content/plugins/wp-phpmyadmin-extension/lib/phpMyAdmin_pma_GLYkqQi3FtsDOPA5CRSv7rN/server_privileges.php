 ...
 */
$strPrivDescAllPrivileges = __PMA_TRANSL('Includes all privileges except GRANT.');
$strPrivDescAlter = __PMA_TRANSL('Allows altering the structure of existing tables.');
$strPrivDescAlterRoutine = __PMA_TRANSL('Allows altering and dropping stored routines.');
$strPrivDescCreateDb = __PMA_TRANSL('Allows creating new databases and tables.');
$strPrivDescCreateRoutine = __PMA_TRANSL('Allows creating stored routines.');
$strPrivDescCreateTbl = __PMA_TRANSL('Allows creating new tables.');
$strPrivDescCreateTmpTable = __PMA_TRANSL('Allows creating temporary tables.');
$strPrivDescCreateUser = __PMA_TRANSL('Allows creating, dropping and renaming user accounts.');
$strPrivDescCreateView = __PMA_TRANSL('Allows creating new views.');
$strPrivDescDelete = __PMA_TRANSL('Allows deleting data.');
$strPrivDescDropDb = __PMA_TRANSL('Allows dropping databases and tables.');
$strPrivDescDropTbl = __PMA_TRANSL('Allows dropping tables.');
$strPrivDescEvent = __PMA_TRANSL('Allows to set up events for the event scheduler.');
$strPrivDescExecute = __PMA_TRANSL('Allows executing stored routines.');
$strPrivDescFile = __PMA_TRANSL('Allows importing data from and exporting data into files.');
$strPrivDescGrantTbl = __PMA_TRANSL(
    'Allows user to give to other users or remove from other users the privileges '
    . 'that user possess yourself.'
);
$strPrivDescIndex = __PMA_TRANSL('Allows creating and dropping indexes.');
$strPrivDescInsert = __PMA_TRANSL('Allows inserting and replacing data.');
$strPrivDescLockTables = __PMA_TRANSL('Allows locking tables for the current thread.');
$strPrivDescMaxConnections = __PMA_TRANSL(
    'Limits the number of new connections the user may open per hour.'
);
$strPrivDescMaxQuestions = __PMA_TRANSL(
    'Limits the number of queries the user may send to the server per hour.'
);
$strPrivDescMaxUpdates = __PMA_TRANSL(
    'Limits the number of commands that change any table or database '
    . 'the user may execute per hour.'
);
$strPrivDescMaxUserConnections = __PMA_TRANSL(
    'Limits the number of simultaneous connections the user may have.'
);
$strPrivDescProcess = __PMA_TRANSL('Allows viewing processes of all users.');
$strPrivDescReferences = __PMA_TRANSL('Has no effect in this MySQL version.');
$strPrivDescReload = __PMA_TRANSL(
    'Allows reloading server settings and flushing the server\'s caches.'
);
$strPrivDescReplClient = __PMA_TRANSL(
    'Allows the user to ask where the slaves / masters are.'
);
$strPrivDescReplSlave = __PMA_TRANSL('Needed for the replication slaves.');
$strPrivDescSelect = __PMA_TRANSL('Allows reading data.');
$strPrivDescShowDb = __PMA_TRANSL('Gives access to the complete list of databases.');
$strPrivDescShowView = __PMA_TRANSL('Allows performing SHOW CREATE VIEW queries.');
$strPrivDescShutdown = __PMA_TRANSL('Allows shutting down the server.');
$strPrivDescSuper = __PMA_TRANSL(
    'Allows connecting, even if maximum number of connections is reached; '
    . 'required for most administrative operations like setting global variables '
    . 'or killing threads of other users.'
);
$strPrivDescTrigger = __PMA_TRANSL('Allows creating and dropping triggers.');
$strPrivDescUpdate = __PMA_TRANSL('Allows changing data.');
$strPrivDescUsage = __PMA_TRANSL('No privileges.');
...
if (!$GLOBALS['dbi']->isSuperuser() && !$GLOBALS['is_grantuser']
    ...
    $response->addHTML(
        Message::error(__PMA_TRANSL('No Privileges'))
            ->getDisplay()
    );
    exit;
}
if (! $GLOBALS['is_grantuser'] && !$GLOBALS['is_createuser']) {
    $response->addHTML(Message::notice(
        __PMA_TRANSL('You do not have privileges to manipulate with the users!')
    )->getDisplay());
}
...
if ($response->isAjax()
    ...
    || $_REQUEST['initial'] === '')
    || (isset($_REQUEST['delete']) && $_REQUEST['delete'] === __PMA_TRANSL('Go')))
