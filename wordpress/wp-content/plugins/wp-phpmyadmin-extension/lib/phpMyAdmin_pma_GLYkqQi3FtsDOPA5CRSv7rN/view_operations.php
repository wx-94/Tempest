...
if (isset($result)) {
    ...
    if (empty($_message->getString())) {
        if ($result) {
            ...
            $_message->addText(__PMA_TRANSL('Error'));
        }
...
<fieldset>
    <legend><?php echo __PMA_TRANSL('Operations'); ?></legend>

    <table>
    <!-- Change view name -->
    <tr><td><?php echo __PMA_TRANSL('Rename view to'); ?></td>
...
<fieldset class="tblFooters">
        ...
        <input type="submit" value="<?php echo __PMA_TRANSL('Go'); ?>" />
</fieldset>
</form>
</div>
<?php
$drop_view_url_params = array_merge(
    ...
    array(
        ...
        'message_to_show' => sprintf(
            __PMA_TRANSL('View %s has been dropped.'),
...
echo '<legend>' , __PMA_TRANSL('Delete data or table') , '</legend>';

echo '<ul>';
echo $operations->getDeleteDataOrTablelink(
    $drop_view_url_params,
    'DROP VIEW',
    __PMA_TRANSL('Delete the view (DROP)'),
    'drop_view_anchor'
);
echo '</ul>';
echo '</fieldset>';
