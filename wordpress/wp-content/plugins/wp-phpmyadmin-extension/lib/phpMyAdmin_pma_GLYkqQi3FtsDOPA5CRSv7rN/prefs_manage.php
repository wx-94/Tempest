...
if (isset($_POST['submit_export'])
    ...
    $settings = $userPreferences->load();
    echo '/* ' . __PMA_TRANSL('phpMyAdmin configuration snippet') . " */\n\n";
    echo '/* ' . __PMA_TRANSL('Paste it to your config.inc.php') . " */\n\n";
    ...
    if (! is_array($config)) {
        if (! isset($error)) {
            $error = __PMA_TRANSL('Could not import configuration');
        }
    } else {
        ...
        if (!$all_ok) {
            ...
            $msg = Message::error(
                __PMA_TRANSL('Configuration contains incorrect data for some fields.')
            );
            $msg->display();
            ...
            echo '<p>';
            echo __PMA_TRANSL('Do you want to import remaining settings?');
            echo '</p>';
...
<?php
Sanitize::printJsValue("PMA_messages['strSavedOn']", __PMA_TRANSL('Saved on: @DATE@'));
?>
</script>
<div id="maincontainer">
    <div id="main_pane_left">
        <div class="group">
<?php
echo '<h2>' , __PMA_TRANSL('Import') , '</h2>'
    ...
    , '<label for="import_text_file">' . __PMA_TRANSL('Import from file') . '</label>'
    , '<div id="opts_import_text_file" class="prefsmanage_opts">'
    , '<label for="input_import_file">' , __PMA_TRANSL('Browse your computer:') , '</label>'
    ...
    , __PMA_TRANSL('Import from browser\'s storage') , '</label>'
    , '<div id="opts_import_local_storage" class="prefsmanage_opts disabled">'
    , '<div class="localStorage-supported">'
    , __PMA_TRANSL('Settings will be imported from your browser\'s local storage.')
    , '<br />'
    , '<div class="localStorage-exists">'
    , __PMA_TRANSL('Saved on: @DATE@')
    , '</div>'
    , '<div class="localStorage-empty">';
Message::notice(__PMA_TRANSL('You have no saved settings!'))->display();
echo  '</div>'
    , '</div>'
    , '<div class="localStorage-unsupported">';
Message::notice(
    __PMA_TRANSL('This feature is not supported by your web browser')
)->display();
echo '</div>'
    , '</div>'
    , '<input type="checkbox" id="import_merge" name="import_merge" />'
    , '<label for="import_merge">'
    , __PMA_TRANSL('Merge with current configuration') . '</label>'
    , '<br /><br />'
...
if (@file_exists('setup/index.php') && ! @file_exists(CONFIG_FILE)) {
            ...
            ?>
            <div class="group">
            <h2><?php echo __PMA_TRANSL('More settings') ?></h2>
            <div class="group-cnt">
                <?php
                echo sprintf(
                    __PMA_TRANSL(
    ...
    <div id="main_pane_right">
        <div class="group">
            <h2><?php echo __PMA_TRANSL('Export'); ?></h2>
            <div class="click-hide-message group-cnt hide">
                <?php
                Message::rawSuccess(
                    __PMA_TRANSL('Configuration has been saved.')
                )->display();
                ?>
            </div>
            <form class="group-cnt prefs-form disableAjax" name="prefs_export"
                ...
                <br />
                <?php
                echo '<input type="submit" name="submit_export" value="' , __PMA_TRANSL(
                    'Go'
                ) , '" />';
                ?>
            </form>
        ...
        <div class="group">
            <h2><?php echo __PMA_TRANSL('Reset'); ?></h2>
            <form class="group-cnt prefs-form disableAjax" name="prefs_reset"
                  action="prefs_manage.php" method="post">
                <?php
                echo Url::getHiddenInputs() , __PMA_TRANSL(
                ...
                <input type="submit" name="submit_clear"
                       value="<?php echo __PMA_TRANSL('Reset'); ?>"/>
            </form>
        </div>
    </div>
    <br class="clearfloat" />
