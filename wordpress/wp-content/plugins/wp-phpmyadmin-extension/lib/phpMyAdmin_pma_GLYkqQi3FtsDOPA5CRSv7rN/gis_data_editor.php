...
echo '<h3>';
printf(
    __PMA_TRANSL('Value for the column "%s"'),
...
echo '<label for="choice">' ,  __PMA_TRANSL("Use OpenStreetMaps as Base Layer") , '</label>';
echo '</div>';
...
echo '<label for="srid">' ,  __PMA_TRANSL('SRID:') , '</label>';
...
for ($a = 0; $a < $geom_count; $a++) {
    ...
    if ($geom_type == 'GEOMETRYCOLLECTION') {
        echo '<br/><br/>';
        printf(__PMA_TRANSL('Geometry %d:'), $a + 1);
        echo '<br/>';
    ...
    if ($type == 'POINT') {
        echo '<br/>';
        echo __PMA_TRANSL('Point:');
        echo '<label for="x">' , __PMA_TRANSL("X") , '</label>';
        echo '<input name="gis_data[' , $a , '][POINT][x]" type="text"'
            , ' value="' , escape($gis_data[$a]['POINT']['x']) , '" />';
        echo '<label for="y">' , __PMA_TRANSL("Y") , '</label>';
        ...
        for ($i = 0; $i < $no_of_points; $i++) {
            echo '<br/>';
            printf(__PMA_TRANSL('Point %d'), $i + 1);
            echo ': ';
            echo '<label for="x">' ,  __PMA_TRANSL("X") , '</label>';
        ...
        echo '<input type="submit"'
            ...
            , ' class="add addPoint" value="' , __PMA_TRANSL("Add a point") , '" />';
        ...
        for ($i = 0; $i < $no_of_lines; $i++) {
            ...
            if ($type == 'MULTILINESTRING') {
                printf(__PMA_TRANSL('Linestring %d:'), $i + 1);
            } else {
                if ($i == 0) {
                    echo __PMA_TRANSL('Outer ring:');
                } else {
                    printf(__PMA_TRANSL('Inner ring %d:'), $i);
                }
            }
            ...
            for ($j = 0; $j < $no_of_points; $j++) {
                echo('<br/>');
                printf(__PMA_TRANSL('Point %d'), $j + 1);
                echo ': ';
                echo '<label for="x">' ,  __PMA_TRANSL("X") , '</label>';
        ...
        $caption = ($type == 'MULTILINESTRING')
            ? __PMA_TRANSL('Add a linestring')
            : __PMA_TRANSL('Add an inner ring');
        echo '<br/>';
        echo '<input type="submit"'
        ...
        for ($k = 0; $k < $no_of_polygons; $k++) {
            echo '<br/>';
            printf(__PMA_TRANSL('Polygon %d:'), $k + 1);
            ...
            echo '<input type="submit"'
                ...
                , ' class="add addLine" value="' , __PMA_TRANSL('Add an inner ring') , '" />';
            echo '<br/>';
        }
        echo '<br/>';
        echo '<input type="submit"'
            , ' name="gis_data[' , $a , '][' , $type , '][add_polygon]"'
            , ' class="add addPolygon" value="' ,  __PMA_TRANSL('Add a polygon') , '" />';
    }
}
if ($geom_type == 'GEOMETRYCOLLECTION') {
    echo '<br/><br/>';
    echo '<input type="submit" name="gis_data[GEOMETRYCOLLECTION][add_geom]"'
        , 'class="add addGeom" value="' , __PMA_TRANSL("Add geometry") , '" />';
}
echo '</div>';
echo '<!-- End of data section -->';

echo '<br/>';
echo '<input type="submit" name="gis_data[save]" value="' , __PMA_TRANSL('Go') , '" />';

echo '<div id="gis_data_output">';
echo '<h3>' , __PMA_TRANSL('Output') , '</h3>';
echo '<p>';
echo __PMA_TRANSL(
